import * as u from "react";
import t, { createContext as fr, useContext as Xr, useState as ue, useRef as Lr, useEffect as Hr, useMemo as Or } from "react";
import { ConfigProvider as kr, Avatar as xr, Badge as Dr, Tooltip as jr, Button as $r, Typography as $e, Checkbox as _r, Input as _e, InputNumber as eo, Radio as ro, Rate as oo, Select as to, Tag as ao, Switch as no, DatePicker as Ar, AutoComplete as io, Upload as er, Slider as lo, Collapse as co, Breadcrumb as so, Dropdown as po, Divider as mo, Row as uo, Col as go, Space as vr, Steps as ho, Pagination as yo, Image as Cr, Tree as bo, Tabs as fo, Alert as ko, Progress as xo } from "antd";
import k, { createGlobalStyle as Ie, styled as G, css as F } from "styled-components";
import wr, { Toaster as Ao } from "react-hot-toast";
import { CloseCircleOutlined as ge, WarningOutlined as vo, CheckCircleOutlined as Co, ExclamationCircleOutlined as wo, CheckCircleFilled as So, CheckOutlined as Sr, SearchOutlined as Er, UploadOutlined as Eo, PlusOutlined as Le, LoadingOutlined as Fo, InboxOutlined as Ko, EditOutlined as Io, EyeOutlined as rr, CloseOutlined as or, MoreOutlined as Fr, RightOutlined as Kr, ExclamationOutlined as Go } from "@ant-design/icons";
import To from "antd/lib/notification";
import { merge as He } from "lodash";
import Ge from "classnames";
import Ro from "antd/lib/typography/Base";
import Bo from "rc-util/lib/omit";
import { ConfigContext as Ir } from "antd/lib/config-provider/";
import { GroupContext as Uo } from "antd/lib/checkbox/Group";
import Yo from "antd/lib/checkbox/style";
import zo from "rc-util/lib/hooks/useMergedState";
import Wo from "antd/lib/config-provider/SizeContext";
import Vo from "rc-util/lib/pickAttrs";
import { RadioGroupContextProvider as Mo } from "antd/lib/radio/context";
import Qo from "antd/lib/radio/style";
import Q from "@ant-design/icons/lib/components/AntdIcon";
import Zo from "react-modal";
import { createRoot as Jo } from "react-dom/client";
import { useLocale as Po } from "antd/lib/locale";
import qo from "react-uuid";
const No = "data:font/woff2;base64,", Xo = "data:font/woff2;base64,", Lo = Ie`
    @font-face {
        font-family: 'Inter';
        font-style: normal;
        font-weight: 100 900;
        font-display: swap;
        src: local('Inter'), url('${No}') format('woff2');
        font-named-instance: 'Regular';
    }

    @font-face {
        font-family: 'Inter';
        font-style: italic;
        font-weight: 100 900;
        font-display: swap;
        src: local('Inter'), url('${Xo}') format('woff2');
        font-named-instance: 'Italic';
    }
`, d = { primary: { blue100: "#E7EDFF", blue200: "#97B9FD", blue300: "#5784FC", blue400: "#0141EC", blue500: "#0B37AF", blue600: "#062064", blue700: "#010819" }, neutral: { typography: { black: "#000000", black60: "#BABABA", white: "#FFFFFF" }, background: "#F7F7F7", border: "#9FB9FD", gray: { gray100: "#F1F3F5", gray200: "#DEE2E6", gray300: "#CED4DA", gray400: "#ADB5BD", gray500: "#868E96", gray600: "#495057", gray700: "#212529" } }, secondary: { mediumGrey: { mediumGrey100: "#F3F7F9", mediumGrey200: "#D1DFE9", mediumGrey300: "#AFC9D9", mediumGrey400: "#8DB1C8", mediumGrey500: "#637C8D", mediumGrey600: "#00254E", mediumGrey700: "#0E1214" }, yellow: { yellow100: "#FFF9EC", yellow200: "#FEE7B1", yellow300: "#FDD671", yellow400: "#FFC100", yellow500: "#B08919", yellow600: "#654E11", yellow700: "#191405" }, orange: { orange100: "#FFF4E6", orange200: "#FFD8A8", orange300: "#FFA94D", orange400: "#FF922B", orange500: "#FD7E14", orange600: "#F76707", orange700: "#360F00" }, pink: { pink100: "#FDF0F7", pink200: "#F8C0DF", pink300: "#F08FC6", pink400: "#E75DAD", pink500: "#A34179", pink600: "#5D2645", pink700: "#170911" }, red: { red100: "#FFEDEB", red200: "#FFB7AF", red300: "#FF8072", red400: "#FF412D", red500: "#C22D20", red600: "#6F1B14", red700: "#1C0705" }, blue: { blue100: "#E7EDFF", blue200: "#97B9FD", blue300: "#5784FC", blue400: "#0141EC", blue500: "#0B37AF", blue600: "#062064", blue700: "#010819" }, green: { green100: "#ECFBF7", green200: "#AFF1DD", green300: "#6CE5C3", green400: "#00D9A9", green500: "#009876", green600: "#025744", green700: "#051511" } } }, $ = { fontSize1: 30, lineHeight1: 1.2, fontSize2: 24, lineHeight2: 1.2, fontSize3: 18, lineHeight3: 1.2, fontSize4: 16, lineHeight4: 1.2 }, y = { fontFamily: "Inter", fontStyle: "normal", defaultFontWeight: 500, mediumfontWeight: 500, regularFontWeight: 400, boldFontWeight: 700, fontSize5: 14, fontSize6: 13, fontSize7: 12, lineHeight5: 1.2, lineHeight6: 1.2, lineHeight7: 1.2 }, r = { token: { fontFamily: y.fontFamily }, color: d, typography: y, headersTypography: $, components: { Checkbox: { fontFamily: y.fontFamily, fontSize: y.fontSize5, fontSizeLG: y.fontSize5, lineHeight: y.lineHeight5, colorBorder: d.neutral.typography.black60, borderRadiusSM: 4, lineWidth: 1, controlInteractiveSize: 16, colorPrimary: d.primary.blue400, colorPrimaryHover: d.primary.blue500 }, Radio: { fontFamily: y.fontFamily, fontSize: y.fontSize5, lineHeight: y.lineHeight5, fontSizeLG: $.fontSize4, dotSize: 12, dotColorDisabled: d.primary.blue200 }, Switch: { fontSize: 20, fontSizeIcon: 12, lineHeight: 1, colorPrimary: d.primary.blue400, colorTextQuaternary: d.neutral.background, colorWhite: d.neutral.typography.white }, Divider: { colorText: d.neutral.typography.black, colorTextHeading: d.neutral.typography.black, fontFamily: y.fontFamily, fontSize: y.fontSize6, fontSizeLG: y.fontSize6, lineHeight: y.lineHeight6, colorSplit: d.neutral.typography.black60 }, InputNumber: { colorBgContainer: d.neutral.typography.white, colorBgContainerDisabled: d.secondary.mediumGrey.mediumGrey100, colorBorder: d.secondary.mediumGrey.mediumGrey200, colorPrimary: d.primary.blue400, colorPrimaryHover: d.primary.blue400, colorTextPlaceholder: d.secondary.mediumGrey.mediumGrey500, colorText: d.primary.blue300, borderRadius: 7, fontFamily: y.fontFamily, fontSize: y.fontSize5 }, Dropdown: { borderRadius: 7, fontFamily: y.fontFamily, fontSize: y.fontSize6 }, Select: { borderRadius: 7, controlHeight: 40, colorBgContainer: d.neutral.typography.white, colorBgContainerDisabled: d.secondary.mediumGrey.mediumGrey100, colorBorder: d.secondary.mediumGrey.mediumGrey200, colorPrimaryHover: d.primary.blue400, colorTextPlaceholder: d.secondary.mediumGrey.mediumGrey500, colorBgElevated: d.secondary.mediumGrey.mediumGrey100, controlItemBgHover: d.primary.blue100, fontFamily: y.fontFamily, fontSize: y.fontSize6 }, Steps: { colorPrimary: d.primary.blue400, colorError: d.secondary.red.red400, fontFamily: y.fontFamily, fontSizeLG: y.fontSize5, fontSize: y.fontSize6, colorText: d.neutral.typography.black, colorTextDescription: d.neutral.typography.black60, colorSplit: d.neutral.background, marginSM: 8 }, Alert: { colorSuccessBg: d.secondary.green.green100, colorSuccessBorder: d.secondary.green.green300, colorInfoBg: d.secondary.blue.blue100, colorInfoBorder: d.secondary.blue.blue300, colorWarningBg: d.secondary.yellow.yellow100, colorWarningBorder: d.secondary.yellow.yellow300, colorErrorBg: d.secondary.red.red100, colorErrorBorder: d.secondary.red.red400, colorText: d.neutral.typography.black, fontFamily: y.fontFamily }, Breadcrumb: { fontFamily: y.fontFamily, fontSize: y.fontSize5, fontSizeIcon: y.fontSize5, itemColor: d.neutral.gray.gray500, separatorColor: d.neutral.gray.gray500, linkColor: d.neutral.gray.gray500, linkHoverColor: d.neutral.typography.black, lastItemColor: d.primary.blue400, colorBgTextHover: "transparent" }, Progress: { colorBgContainer: d.neutral.background, colorError: d.secondary.red.red400, colorInfo: d.primary.blue400, colorSuccess: d.secondary.green.green400, fontFamily: y.fontFamily, fontSize: y.fontSize7 }, Pagination: { fontFamily: y.fontFamily, fontSize: y.fontSize5, fontSizeSM: y.fontSize5, fontWeightStrong: y.mediumfontWeight, colorBgContainer: d.neutral.typography.white, colorBgContainerDisabled: d.neutral.background, colorBgTextActive: d.neutral.typography.black60, colorTextDisabled: d.neutral.typography.black60, colorPrimary: d.primary.blue400, colorPrimaryBorder: d.primary.blue400, colorPrimaryHover: d.primary.blue400, controlHeight: 32, controlHeightSM: 24 }, DatePicker: { colorBgContainer: d.neutral.typography.white, colorBgContainerDisabled: d.secondary.mediumGrey.mediumGrey100, colorBorder: d.secondary.mediumGrey.mediumGrey200, colorPrimary: d.primary.blue400, colorPrimaryHover: d.primary.blue400, colorLink: d.primary.blue400, colorLinkHover: d.primary.blue500, controlItemBgHover: d.secondary.mediumGrey.mediumGrey100, controlItemBgActive: d.primary.blue100, colorTextPlaceholder: d.secondary.mediumGrey.mediumGrey500, colorText: d.primary.blue300, borderRadius: 7, borderRadiusSM: 2, fontFamily: y.fontFamily, fontSize: y.fontSize5 } } }, Ho = { background: "initial", padding: "initial", color: "initial", boxShadow: "initial", lineHeight: "initial", display: "initial", alignItems: "initial", borderRadius: "initial" }, Gr = () => t.createElement(Ao, { position: "bottom-center", toastOptions: { style: Ho } }), j = { color: { on: r.color.primary.blue400, default: r.color.neutral.typography.black }, backgroundColor: { on: r.color.primary.blue100, default: "transparent" }, borderRadius: { default: 0, on: 7 }, width: 32, padding: "8px" }, Oo = G.span`
    color: ${(e) => e.$on ? j.color.on : e.color || j.color.default};
    border-radius: ${(e) => e.$on ? j.borderRadius.on : j.borderRadius.default}px;
    background-color: ${(e) => e.$on ? j.backgroundColor.on : j.backgroundColor.default};
    padding: ${j.padding};

    ${(e) => e.$isClickable && F`
            cursor: pointer;
        `}

    &:hover {
        color: ${(e) => e.$hoverable || e.$on ? j.color.on : j.color.default};
        border-radius: ${(e) => e.$hoverable || e.$on ? j.borderRadius.on : j.borderRadius.default}px;
        background-color: ${(e) => e.$hoverable || e.$on ? j.backgroundColor.on : j.backgroundColor.default};
    }
`, P = ({ className: e, on: o, hoverable: a, icon: i, onClick: l, ...c }) => t.createElement(Oo, { className: "kit-icon " + e, $on: o, $hoverable: a, $isClickable: l !== void 0, ...c }, i), ae = (P.displayName = "KitIcon", r).color, Tr = fr(void 0);
function El() {
  var e = Xr(Tr);
  if (e === void 0)
    throw new Error("useKitNotification must be inside a context");
  return e;
}
const Do = ({ children: e }) => {
  var [o, a] = To.useNotification();
  return t.createElement(Tr.Provider, { value: jo(o) }, e, a);
}, jo = (e) => ({ kitNotification: { error: (o) => {
  e.error({ ...o, icon: t.createElement(P, { style: { color: ae.secondary.red.red400, backgroundColor: ae.secondary.red.red100 }, icon: t.createElement(ge, null), on: !0 }) });
}, warning: (o) => {
  e.warning({ ...o, icon: t.createElement(P, { style: { color: ae.secondary.orange.orange400, backgroundColor: ae.secondary.orange.orange100 }, icon: t.createElement(vo, null), on: !0 }) });
}, success: (o) => {
  e.success({ ...o, icon: t.createElement(P, { style: { color: ae.secondary.green.green400, backgroundColor: ae.secondary.green.green100 }, icon: t.createElement(Co, null), on: !0 }) });
}, info: (o) => {
  e.info({ ...o, icon: t.createElement(P, { style: { color: ae.primary.blue400, backgroundColor: ae.primary.blue100 }, icon: t.createElement(wo, null), on: !0 }) });
}, open: (o) => {
  e.open({ ...o });
}, destroy: (o) => {
  e.destroy(o);
} } }), { typography: ye, color: Y } = r, $o = Ie`
    .ant-dropdown,
    .ant-dropdown-menu.ant-dropdown-menu-submenu {
        padding: 10px;
        border-radius: 7px;
        font-family: ${ye.fontFamily};
        font-weight: ${ye.mediumfontWeight};
        background-color: ${Y.secondary.mediumGrey.mediumGrey100};
        color: ${Y.secondary.mediumGrey.mediumGrey500};
        box-shadow: 0px 3px 14px 0px rgba(0, 0, 0, 0.30);

        .ant-dropdown-arrow {
            &::before, &::after {
                background-color: ${Y.secondary.mediumGrey.mediumGrey100};
                box-shadow: 2px 1px 5px rgba(0, 0, 0, 0.15);
            }
        }

        .ant-dropdown-menu {
            overflow: visible;
            padding: 0px;
            background-color: ${Y.secondary.mediumGrey.mediumGrey100};
            box-shadow: none;

            .ant-dropdown-menu-item-group:first-of-type .ant-dropdown-menu-item-group-title {
                padding-top: 0px;
            }

            .ant-dropdown-menu-item-group-title {
                padding: 8px 0;
                color: ${Y.neutral.typography.black};
                font-size: ${ye.fontSize7}px;
                font-weight: ${ye.boldFontWeight};
            }

            .ant-dropdown-menu-item-group-list {
                margin: 0 0 0 5px;
            }

            .ant-dropdown-menu-item,
            .ant-dropdown-menu-submenu {
                border-radius: 7px;
                border: none;
                min-height: 32px;
                font-weight: ${ye.mediumfontWeight};
                color: ${Y.secondary.mediumGrey.mediumGrey500};
                padding: 0 10px;

                &:not(:last-child) {
                    margin-bottom: 5px;
                }

                &.ant-dropdown-menu-item-active {
                    background-color: ${Y.primary.blue100};
                    color: ${Y.primary.blue400};
                }

                &.ant-dropdown-menu-item-selected {
                    background-color: ${Y.primary.blue100};
                }

                &.ant-dropdown-menu-item-disabled {
                    background-color: ${Y.secondary.mediumGrey.mediumGrey100};
                    color: ${Y.secondary.mediumGrey.mediumGrey400};
                    cursor: not-allowed;
                }
            }

            .ant-dropdown-menu-submenu {
                display: flex;
                align-items: center;

                .ant-dropdown-menu-submenu-title {
                    background-color: initial;
                    padding: 0;
                    font-weight: ${ye.mediumfontWeight};
                    color: ${Y.secondary.mediumGrey.mediumGrey500};
                    width: 100%;

                    .ant-dropdown-menu-title-content {
                        padding-right: 8px;
                    }

                    .ant-dropdown-menu-submenu-expand-icon {
                        position: initial;
                        float: right;

                        .anticon {
                            color: ${Y.secondary.mediumGrey.mediumGrey500};
                        }
                    }
                }

                &.ant-dropdown-menu-submenu-active {
                    background-color: ${Y.primary.blue100};
                    color: ${Y.primary.blue400};

                    .ant-dropdown-menu-submenu-title,
                    .ant-dropdown-menu-submenu-expand-icon .anticon {
                        color: ${Y.primary.blue400};
                    }
                }

                &.ant-dropdown-menu-submenu-disabled {
                    .ant-dropdown-menu-submenu-title {
                        background-color: ${Y.secondary.mediumGrey.mediumGrey100};
                        color: ${Y.secondary.mediumGrey.mediumGrey400};
                        cursor: not-allowed;

                        .ant-dropdown-menu-submenu-expand-icon .anticon {
                            background-color: ${Y.secondary.mediumGrey.mediumGrey100};
                            color: ${Y.secondary.mediumGrey.mediumGrey400};
                        }
                    }
                }
            }

            .ant-dropdown-menu-item-divider {
                background-color: ${Y.secondary.mediumGrey.mediumGrey200};
            }

            a {
                transition: all 0s;
            }
        }
    }
`, _o = k.div`
    display: inline-block;
    vertical-align: middle;

    div {
        margin-left: 4px;
        width: 16px;
        height: 16px;
        border-radius: 2px;
    }
`, tr = k.div`
    width: 100%;
    box-sizing: border-box;
    display: inline-block;
    vertical-align: middle;
    overflow: hidden;
    text-overflow: ellipsis;
    padding-left: 10px;
    padding-right: 10px;
`, et = Ie`
    .ant-select-dropdown {
        background: transparent;
        overflow: visible;
        padding: 0;
        border-radius: 0;
        box-shadow: none;
        font-family: ${r.typography.fontFamily};
        font-weight: ${r.typography.mediumfontWeight};
        color: ${r.color.secondary.mediumGrey.mediumGrey500};

        .ant-select-item, .ant-select-item-option, .ant-select-item-option-selected {
            border-radius: 7px;
            border: none;
            min-height: 32px;
            font-weight: ${r.typography.mediumfontWeight};
            color: ${r.color.secondary.mediumGrey.mediumGrey500};

            .ant-select-item-option-content {
                display: flex;
                align-items: center;
            }
        }

        .ant-select-item-group {
            color: ${r.color.neutral.typography.black};
            font-weight: ${r.typography.boldFontWeight};
            padding: 8px 0;
        }
    }

    .rc-virtual-list-holder {
        background-color: ${r.color.secondary.mediumGrey.mediumGrey100};
        padding-left: 0px;
        overflow: hidden;
        border-radius: 7px ;

        .ant-select-item.ant-select-item-option {
            padding-left: 10px;
        }
    }

    .kit-select-dropdown-content {
        box-shadow: 0px 3px 14px 0px rgba(0, 0, 0, 0.30);
        background-color: ${r.color.secondary.mediumGrey.mediumGrey100};
        padding: 10px;
        overflow: hidden;


        .ant-select-item.ant-select-item-option {
            padding: 0;
            margin: 0;
            border: none;
            background: transparent;

            &:not(:last-child) {
                margin-bottom: 5px;
            }

            &.ant-select-item-option-grouped {
                margin-left: 5px;
            }

            &[title] {
                padding-left: 10px;
            }

            .kit-select-option {
                align-self: center;
                width: 100%;

                .kit-select-option-icon {
                    vertical-align: middle;
                }

                .kit-select-option-icon + .kit-select-option-label,
                .kit-select-option-badge + .kit-select-option-label {
                    width: calc(100% - 20px);
                }
            }

            .kit-icon {
                padding: 5px;
            }
        }
        .ant-select-item.ant-select-item-option-selected, .ant-select-item.ant-select-item-option-active {
            background: ${r.color.primary.blue100};
            font-weight: ${r.typography.mediumfontWeight};
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        .ant-select-item.ant-select-item-option-disabled {
            opacity: .5;
        }

        .ant-select-item.ant-select-item-option-active {
            color: ${r.color.primary.blue400};
        }
    }

    .kit-select-dropdown-bottom {
        .kit-select-dropdown-content {
            border-radius: 0 0 7px 7px ;
            transform: translateX(-1px) translateY(-3px);
        }

        &.ant-select-dropdown-placement-topLeft .kit-select-dropdown-content,
        &.ant-select-dropdown-placement-topLeft .kit-select-dropdown-content,
        &.ant-select-dropdown-placement-topLeft .rc-virtual-list-holder,
        &.ant-select-dropdown-placement-topLeft .rc-virtual-list-holder {
            border-radius: 7px 7px 0 0 ;
        }
    }

    .kit-select-dropdown-top {
        .kit-select-dropdown-content {
            border-radius: 7px 7px 0 0 ;
            transform: translateX(-1px) translateY(3px);
        }

        &.ant-select-dropdown-placement-bottomLeft .kit-select-dropdown-content,
        &.ant-select-dropdown-placement-bottomRight .kit-select-dropdown-content,
        &.ant-select-dropdown-placement-bottomLeft .rc-virtual-list-holder,
        &.ant-select-dropdown-placement-bottomRight .rc-virtual-list-holder {
            border-radius: 0 0 7px 7px ;
        }
    }
`, rt = F`
    &.ant-select.ant-select-compact-item {
        &:not(.ant-select-compact-last-item) {
            margin-inline-end: 0px;
        }

        &.ant-select-compact-first-item:not(.ant-select-compact-last-item):not(.ant-select-open) {
            border-radius: 7px 0 0 7px;
        }
    }

    &.ant-select-focused.ant-select:not(.ant-select-disabled):not(.ant-select-customize-input) {
        .ant-select-selector {
            box-shadow: none;
        }
    }

    &.ant-select-open {
        &:not(.ant-select-compact-item) {
            &.ant-select-bottom {
                border-radius: 7px 7px 0 0;
            }

            &.ant-select-top {
                border-radius: 0 0 7px 7px;
            }
        }

        &.ant-select-compact-item {
            &.ant-select-bottom {
                border-radius: 7px 7px 0 0;
            }

            &.ant-select-top {
                border-radius: 0 0 7px 7px;
            }
        }

        &.ant-select-compact-item.ant-select-compact-first-item:not(.ant-select-compact-last-item) {
            &.ant-select-bottom {
                border-radius: 7px 0 0 0;
            }

            &.ant-select-top {
                border-radius: 0 0 0 7px;
            }
        }
    }

    &.ant-select:not(.ant-select-customize-input) {
        display: grid;
        min-height: 40px;
        grid-template-areas: 'input clear arrow';
        grid-template-columns: calc(100% - 60px) 30px 30px;
        position: relative;
        background-color: #ffffff;
        border: 1px solid #d1dfe9;
        border-radius: 7px;
        transition: all 0.2s cubic-bezier(0.645, 0.045, 0.355, 1);

        &:hover,
        &.ant-select-focused {
            border-color: ${r.color.primary.blue400};
        }

        .ant-select-selector {
            grid-area: input;
            padding: 4px 9px;
            background-color: inherit;
            border: none;
            transition: none;
            min-height: 38px;

            .ant-select-selection-overflow {
                gap: 4px 0px;

                .ant-select-selection-overflow-item {
                    max-width: calc(100% - 4px);

                    > span,
                    .ant-tag {
                        max-width: 100%;
                    }

                    .ant-tag .kit-select-option {
                        width: calc(100% - 20px);

                        .kit-select-option-label {
                            padding-right: 0;
                        }
                    }

                    &.ant-select-selection-overflow-item-rest .ant-select-selection-item {
                        background: none;
                        margin: 0;
                        padding-inline-start: 0;
                        padding-inline-end: 0;
                    }
                }
            }

            .ant-select-selection-item,
            .ant-select-selection-placeholder {
                display: flex;
                align-items: center;
                font-weight: ${r.typography.mediumfontWeight};
                color: ${r.color.primary.blue300};
                padding-inline-end: 0;
            }

            .ant-select-selection-placeholder {
                font-weight: ${r.typography.regularFontWeight};
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }

            .ant-select-selection-item .kit-icon {
                padding: 5px;
            }

            .ant-select-selection-search input {
                font-weight: ${r.typography.mediumfontWeight};
                color: ${r.color.primary.blue300};
            }

            .kit-select-option {
                display: inline-block;
                align-self: center;
                width: 100%;

                .kit-select-option-icon {
                    vertical-align: middle;
                }
            }

            .ant-tag {
                .kit-select-option {
                    .kit-select-option-icon {
                        padding: 0;
                    }

                    .kit-select-option-color {
                        margin-left: 0;
                    }
                }
            }
        }

        .ant-select-arrow {
            grid-area: arrow;
            position: relative;
            top: 48%;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
            inset-inline-end: 0;
        }

        .ant-select-clear {
            grid-area: clear;
            position: relative;
            display: flex;
            inset-inline-end: 0;
            font-size: 12px;
            transform: none;
            transition: none;
            opacity: 1;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        &.ant-select-disabled {
            background-color: ${r.color.secondary.mediumGrey.mediumGrey100};
            color: ${r.color.secondary.mediumGrey.mediumGrey400};
            border-color: ${r.color.secondary.mediumGrey.mediumGrey200};
            cursor: not-allowed;

            .ant-select-selection-item {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }

            .ant-select-selection-placeholder {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }

            .ant-select-arrow,
            .ant-select-clear {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }
        }

        &.ant-select-status-warning {
            background-color: ${r.color.secondary.orange.orange100};
            border-color: ${r.color.secondary.orange.orange400};
            color: ${r.color.secondary.orange.orange400};

            .ant-select-selection-item {
                color: ${r.color.secondary.orange.orange500};
            }

            .ant-select-selection-placeholder {
                color: ${r.color.secondary.orange.orange400};
            }

            .ant-select-arrow,
            .ant-select-clear {
                color: ${r.color.secondary.orange.orange400};
                background-color: ${r.color.secondary.orange.orange100};
            }
        }

        &.ant-select-status-error {
            background-color: ${r.color.secondary.red.red100};
            border-color: ${r.color.secondary.red.red400};
            color: ${r.color.secondary.red.red400};

            .ant-select-selection-item {
                color: ${r.color.secondary.red.red400};
            }

            .ant-select-selection-placeholder {
                color: ${r.color.secondary.red.red400};
            }

            .ant-select-arrow,
            .ant-select-clear {
                color: ${r.color.secondary.red.red400};
                background-color: ${r.color.secondary.red.red100};
            }
        }

        &.ant-select-focused:not(.ant-select-customize-input) {
            border-style: dashed;

            .ant-select-selector span.ant-select-selection-placeholder {
                color: transparent;
            }
        }
    }
`, ot = Ie`
    .ant-picker-dropdown {
        ul.ant-picker-ranges {
            margin-block-start: 0px;
        }

        .ant-picker-now .ant-picker-now-btn {
            color: ${r.color.primary.blue400};

            &:hover {
                color: ${r.color.primary.blue500};
            }
        }

        .ant-picker-ok button {
            border-radius: 20px;
            padding: 0px 8px;
            box-shadow: none;

            background-color: ${r.color.primary.blue400};
            border-color: ${r.color.primary.blue400};
            color: ${r.color.neutral.typography.white};

            &:hover:not(.ant-btn-disabled) {
                background-color: ${r.color.primary.blue500};
                border-color: ${r.color.primary.blue500};
            }

            &:active:not(.ant-btn-disabled) {
                background-color: ${r.color.primary.blue600};
                border-color: ${r.color.primary.blue600};
            }

            &[disabled] {
                background-color: ${r.color.primary.blue100};
                border-color: ${r.color.primary.blue100};
                color: ${r.color.primary.blue200};
            }
        }
    }
`, { color: Re, headersTypography: tt, typography: Qe } = r, at = Ie`
    .ant-notification-notice-info .ant-notification-notice-content .ant-notification-notice-message {
        color: ${Re.primary.blue400};
    }

    .ant-notification-notice-success .ant-notification-notice-content .ant-notification-notice-message {
        color: ${Re.secondary.green.green400};
    }

    .ant-notification-notice-error .ant-notification-notice-content .ant-notification-notice-message {
        color: ${Re.secondary.red.red400};
    }

    .ant-notification-notice-warning .ant-notification-notice-content .ant-notification-notice-message {
        color: ${Re.secondary.orange.orange400};
    }

    .ant-notification {
        .ant-notification-notice-content .ant-notification-notice-with-icon .ant-notification-notice-icon {
            font-size: 16px;
            margin-top: 10px;
        }

        .ant-notification-notice {
            font-family: ${Qe.fontFamily};
            padding: 16px;
            font-size: 10px;
            .ant-notification-notice-with-icon {
                .ant-notification-notice-message {
                    font-size: ${tt.fontSize4}px;
                    font-weight: ${Qe.mediumfontWeight};
                    margin-inline-start: 48px;
                    padding-right: 32px;
                }
                .ant-notification-notice-description {
                    margin-inline-start: 48px;
                    font-size: ${Qe.fontSize5}px;
                    line-height: 22px;
                    margin-bottom: 8px;
                }
            }

            .ant-notification-notice-close {
                top: 16px;
                inset-inline-end: 16px;
                font-size: 14px;
            }
        }
    }
`, nt = { primary: { blue100: "#E7EDFF", blue200: "#97B9FD", blue300: "#5784FC", blue400: "#0141EC", blue500: "#0B37AF", blue600: "#062064", blue700: "#010819" }, neutral: { typography: { black: "#000000", black60: "#BABABA", white: "#FFFFFF" }, background: "#F7F7F7", border: "#9FB9FD", gray: { gray100: "#F1F3F5", gray200: "#DEE2E6", gray300: "#CED4DA", gray400: "#ADB5BD", gray500: "#868E96", gray600: "#495057", gray700: "#212529" } }, secondary: { mediumGrey: { mediumGrey100: "#F3F7F9", mediumGrey200: "#D1DFE9", mediumGrey300: "#AFC9D9", mediumGrey400: "#8DB1C8", mediumGrey500: "#637C8D", mediumGrey600: "#00254E", mediumGrey700: "#0E1214" }, yellow: { yellow100: "#FFF9EC", yellow200: "#FEE7B1", yellow300: "#FDD671", yellow400: "#FFC100", yellow500: "#B08919", yellow600: "#654E11", yellow700: "#191405" }, orange: { orange100: "#FFF4E6", orange200: "#FFD8A8", orange300: "#FFA94D", orange400: "#FF922B", orange500: "#FD7E14", orange600: "#F76707", orange700: "#360F00" }, pink: { pink100: "#FDF0F7", pink200: "#F8C0DF", pink300: "#F08FC6", pink400: "#E75DAD", pink500: "#A34179", pink600: "#5D2645", pink700: "#170911" }, red: { red100: "#FFEDEB", red200: "#FFB7AF", red300: "#FF8072", red400: "#FF412D", red500: "#C22D20", red600: "#6F1B14", red700: "#1C0705" }, blue: { blue100: "#E7EDFF", blue200: "#97B9FD", blue300: "#5784FC", blue400: "#0141EC", blue500: "#0B37AF", blue600: "#062064", blue700: "#010819" }, green: { green100: "#ECFBF7", green200: "#AFF1DD", green300: "#6CE5C3", green400: "#00D9A9", green500: "#009876", green600: "#025744", green700: "#051511" } } }, it = { fontFamily: "Inter", fontStyle: "normal", defaultFontWeight: 500, mediumfontWeight: 500, regularFontWeight: 400, boldFontWeight: 700, fontSize1: 30, fontSize2: 24, fontSize3: 18, fontSize4: 16, fontSize5: 14, fontSize6: 13, fontSize7: 12, lineHeight1: 1.2, lineHeight2: 1.2, lineHeight3: 1.2, lineHeight4: 1.2, lineHeight5: 1.2, lineHeight6: 1.2, lineHeight7: 1.2 }, Fe = { radius: { square: 0, xxs: 2, xs: 4, s: 8, m: 16, l: 24, xl: 32, xxl: 40, pills: 1e3 }, width: 0 }, lt = { none: 0, xxs: 4, xs: 8, s: 16, m: 24, l: 32, xl: 40, xxl: 48, huge: 56, veryHuge: 64 }, q = { colors: nt, border: Fe, spacing: lt, typography: it }, fe = q.colors, dt = { colors: { star: { active: { default: fe.secondary.yellow.yellow300 } } } }, ct = { colors: { star: { default: fe.secondary.mediumGrey.mediumGrey100, disabled: fe.secondary.mediumGrey.mediumGrey100, active: { red: fe.secondary.red.red300, green: fe.secondary.green.green300, blue: fe.secondary.blue.blue300 } } } }, st = (e) => (e = e.colors, { starColor: e.star.active.default }), pt = { ...st(dt), ...ct }, { colors: ir, typography: lr, border: mt } = q, ut = { colors: { background: { default: ir.primary.blue600 }, typography: { default: ir.neutral.typography.white } }, border: { radius: mt.radius.s }, typography: { fontFamily: lr.fontFamily, fontSize: lr.fontSize7 } }, gt = (o) => {
  var { colors: o, border: a, typography: i } = o;
  return { colorBgSpotlight: o.background.default, colorTextLightSolid: o.typography.default, borderRadius: a.radius, fontSize: i.fontSize, fontFamily: i.fontFamily };
}, ht = { ...gt(ut) }, { colors: W, typography: Ze, border: yt } = q, bt = { colors: { background: { default: W.primary.blue100 }, typography: { default: W.neutral.typography.black } }, border: { radius: yt.radius.s } }, ft = { colors: { red: { background: { default: W.secondary.red.red400 }, typography: { default: W.neutral.typography.white, hover: W.secondary.red.red200, closeIcon: W.neutral.typography.white } }, green: { background: { default: W.secondary.green.green400 }, typography: { default: W.neutral.typography.white, hover: W.secondary.green.green200, closeIcon: W.neutral.typography.white } }, blue: { background: { default: W.secondary.blue.blue400 }, typography: { default: W.neutral.typography.white, hover: W.secondary.blue.blue200, closeIcon: W.neutral.typography.white } }, blueInvert: { background: { default: W.secondary.blue.blue100 }, typography: { default: W.secondary.blue.blue400, hover: W.secondary.blue.blue300, closeIcon: W.secondary.blue.blue400 } } }, typography: { fontFamily: Ze.fontFamily, fontSize: Ze.fontSize7, fontWeight: Ze.regularFontWeight } }, kt = (o) => {
  var { colors: o, border: a } = o;
  return { defaultBg: o.background.default, defaultColor: o.typography.default, borderRadiusSM: a.radius };
}, xt = { ...kt(bt), ...ft }, { colors: Be, spacing: dr } = q, At = { colors: { typography: { default: Be.secondary.mediumGrey.mediumGrey500, disabled: Be.secondary.mediumGrey.mediumGrey400, warning: Be.secondary.orange.orange400, error: Be.secondary.red.red400 } }, gap: { label: dr.xs, helper: dr.xs } }, vt = { ...At }, { colors: b, typography: ke, border: Ct } = q, wt = { colors: { background: { default: b.neutral.typography.white, disabled: b.secondary.mediumGrey.mediumGrey100 }, border: { default: b.secondary.mediumGrey.mediumGrey200, hover: b.primary.blue400 }, typography: { content: b.primary.blue300, placeholder: b.secondary.mediumGrey.mediumGrey500 } }, border: { radius: Ct.radius.s }, typography: { fontFamily: ke.fontFamily, fontSize: ke.fontSize5 } }, St = { colors: { background: { warning: b.secondary.orange.orange100, error: b.secondary.red.red100 }, prefix: { default: b.secondary.mediumGrey.mediumGrey500, disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange400, error: b.secondary.red.red400 }, suffix: { default: b.secondary.mediumGrey.mediumGrey500, disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange400, error: b.secondary.red.red400 }, clearIcon: { default: b.secondary.mediumGrey.mediumGrey500, disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange500, error: b.secondary.red.red400 }, showCount: { default: b.secondary.mediumGrey.mediumGrey300, disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange300, error: b.secondary.red.red300 }, border: { disabled: b.secondary.mediumGrey.mediumGrey200, warning: b.secondary.orange.orange400, error: b.secondary.red.red400 }, typography: { content: { disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange500, error: b.secondary.red.red400 }, placeholder: { disabled: b.secondary.mediumGrey.mediumGrey400, warning: b.secondary.orange.orange500, error: b.secondary.red.red400 } } }, typography: { content: { fontWeight: ke.mediumfontWeight }, placeholder: { fontWeight: ke.regularFontWeight }, showCount: { fontSize: ke.fontSize7, fontWeight: ke.regularFontWeight } } }, Et = (o) => {
  var { colors: o, border: a, typography: i } = o;
  return { colorBgContainer: o.background.default, colorBgContainerDisabled: o.background.disabled, colorBorder: o.border.default, colorPrimaryHover: o.border.hover, colorTextPlaceholder: o.typography.placeholder, colorText: o.typography.content, borderRadius: a.radius, fontFamily: i.fontFamily, fontSize: i.fontSize };
}, Ft = { ...Et(wt), ...St }, xe = q.colors, Kt = { colors: { background: { default: xe.neutral.typography.black60, blueInvert: xe.primary.blue100 }, typography: { default: xe.neutral.typography.white, blueInvert: xe.secondary.blue.blue200 } } }, It = { ...Kt }, Gt = { colors: { background: { default: xe.primary.blue100 }, typography: { default: xe.secondary.blue.blue200 } } }, Tt = { ...Gt }, { colors: X, typography: Rt } = q, Bt = { colors: { background: { primary: X.primary.blue400, error: X.secondary.red.red400, success: X.secondary.green.green400, warning: X.secondary.yellow.yellow400 } }, typography: { fontFamily: Rt.fontFamily } }, Ut = { colors: { background: { default: X.secondary.red.red400, green: X.secondary.green.green400, blue: X.primary.blue400, blueInvert: X.primary.blue100, gray: X.neutral.background }, typography: { default: X.neutral.typography.white, green: X.neutral.typography.white, blue: X.neutral.typography.white, blueInvert: X.secondary.blue.blue400, gray: X.neutral.typography.black60 } } }, Yt = (o) => {
  var { colors: o, typography: a } = o;
  return { fontFamily: a.fontFamily, colorError: o.background.error, colorSuccess: o.background.success, colorPrimary: o.background.primary, colorWarning: o.background.warning };
}, zt = { ...Yt(Bt), ...Ut }, { colors: J, typography: pe } = q, Wt = { card: { colors: { background: { default: J.neutral.typography.white, disabled: J.primary.blue100 }, border: { default: "transparent", hover: J.primary.blue400 } }, border: { radius: Fe.radius.s }, typography: { fontFamily: pe.fontFamily } }, colorBar: { thickness: 8, border: { radius: Fe.radius.s } }, select: { colors: { background: { disabled: J.primary.blue100 }, typography: { default: J.primary.blue400, disabled: J.primary.blue200 } } }, expend: { colors: { typography: { default: J.primary.blue600, hover: J.primary.blue400 } } }, image: { colors: { border: { default: J.neutral.typography.black60 } }, border: { radius: Fe.radius.s } }, icon: { colors: { border: { default: J.neutral.typography.black60 } }, border: { radius: Fe.radius.s } }, title: { colors: { typography: { default: J.primary.blue600, disabled: J.primary.blue200 } }, typography: { fontSize: pe.fontSize6, fontWeight: pe.boldFontWeight } }, description: { colors: { typography: { default: J.primary.blue600, disabled: J.primary.blue200 } }, typography: { fontSize: pe.fontSize5, fontWeight: pe.regularFontWeight } }, footer: { colors: { typography: { default: J.primary.blue600, disabled: J.primary.blue200 } }, typography: { fontSize: pe.fontSize5, fontWeight: pe.boldFontWeight } } }, Vt = { ...Wt }, { colors: n, typography: f, border: Ce } = q, Mt = { typography: { fontFamily: f.fontFamily, fontSize: f.fontSize5, lineHeight: f.lineHeight5 }, border: { radius: Ce.radius.pills } }, Qt = { colors: { typography: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.neutral.typography.white, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey400, ghost: n.primary.blue400, danger: { default: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey400 } }, background: { default: n.primary.blue400, hover: n.primary.blue500, active: n.primary.blue600, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey100, ghost: n.neutral.typography.white, danger: { default: n.secondary.red.red400, hover: n.secondary.red.red500, active: n.secondary.red.red600, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey100 } }, border: { default: n.primary.blue400, hover: n.primary.blue500, active: n.primary.blue600, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey200, danger: { default: n.secondary.red.red400, hover: n.secondary.red.red500, active: n.secondary.red.red600, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey200 } } }, typography: { fontWeight: f.regularFontWeight, iconSize: { xs: f.fontSize7, s: f.fontSize6, m: f.fontSize5, l: f.fontSize4, xl: f.fontSize3 } }, compact: { border: { radius: Ce.radius.s } } }, Zt = { colors: { typography: { default: n.primary.blue400, hover: n.primary.blue400, active: n.primary.blue400, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey400, danger: { default: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey400 } }, background: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.primary.blue100, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100, danger: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.secondary.red.red100, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100 } }, border: { default: n.primary.blue100, hover: n.primary.blue400, active: n.primary.blue400, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey200, danger: { default: n.secondary.red.red100, hover: n.secondary.red.red400, active: n.secondary.red.red400, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey200 } } }, typography: { fontWeight: f.regularFontWeight, iconSize: { xs: f.fontSize7, s: f.fontSize6, m: f.fontSize5, l: f.fontSize4, xl: f.fontSize3 } }, compact: { border: { radius: Ce.radius.s } } }, Jt = { colors: { typography: { default: n.primary.blue400, hover: n.primary.blue400, active: n.primary.blue400, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey400, danger: { default: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey400 } }, background: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.neutral.typography.white, focus: n.neutral.typography.white, disabled: n.neutral.typography.white, danger: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.neutral.typography.white, focus: n.neutral.typography.white, disabled: n.neutral.typography.white } }, border: { default: n.primary.blue400, hover: n.primary.blue400, active: n.primary.blue400, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey200, danger: { default: n.secondary.red.red400, hover: n.secondary.red.red400, active: n.secondary.red.red400, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey200 } } }, typography: { fontWeight: f.regularFontWeight, iconSize: { xs: f.fontSize7, s: f.fontSize6, m: f.fontSize5, l: f.fontSize4, xl: f.fontSize3 } }, compact: { border: { radius: Ce.radius.s } } }, Pt = { colors: { typography: { default: n.primary.blue500, hover: n.primary.blue500, active: n.primary.blue500, focus: n.primary.blue500, disabled: n.secondary.mediumGrey.mediumGrey400, danger: { default: n.secondary.red.red500, disabled: n.secondary.mediumGrey.mediumGrey400 }, iconCheck: n.primary.blue400 }, background: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.primary.blue100, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100, danger: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.secondary.red.red100, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100 } }, border: { default: n.primary.blue100, hover: n.primary.blue400, active: n.primary.blue400, focus: n.primary.blue400, disabled: n.secondary.mediumGrey.mediumGrey200, danger: { default: n.secondary.red.red100, hover: n.secondary.red.red400, active: n.secondary.red.red400, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey200 } } }, typography: { fontWeight: f.regularFontWeight, iconSize: { xs: f.fontSize7, s: f.fontSize6, m: f.fontSize5, l: f.fontSize4, xl: f.fontSize3 } }, compact: { border: { radius: Ce.radius.s } } }, qt = () => Object.keys(n.secondary).reduce((e, o) => {
  var a = { colors: { typography: { default: n.secondary[o][o + "500"], hover: n.secondary[o][o + "500"], active: n.secondary[o][o + "500"], focus: n.secondary[o][o + "500"], disabled: n.secondary.mediumGrey.mediumGrey400, danger: { default: n.secondary.red.red500, disabled: n.secondary.mediumGrey.mediumGrey400 }, iconCheck: n.secondary[o][o + "400"] }, background: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.secondary[o][o + "100"], focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100, danger: { default: n.neutral.typography.white, hover: n.neutral.typography.white, active: n.secondary.red.red100, focus: n.neutral.typography.white, disabled: n.secondary.mediumGrey.mediumGrey100 } }, border: { default: n.secondary[o][o + "100"], hover: n.secondary[o][o + "400"], active: n.secondary[o][o + "400"], focus: n.secondary[o][o + "400"], disabled: n.secondary.mediumGrey.mediumGrey200, danger: { default: n.secondary.red.red100, hover: n.secondary.red.red400, active: n.secondary.red.red400, focus: n.secondary.red.red400, disabled: n.secondary.mediumGrey.mediumGrey200 } } }, typography: { fontWeight: f.regularFontWeight, iconSize: { xs: f.fontSize7, s: f.fontSize6, m: f.fontSize5, l: f.fontSize4, xl: f.fontSize3 } }, compact: { border: { radius: Ce.radius.s } } };
  return { ...e, [o]: a };
}, {}), Nt = (o) => {
  var { typography: o, border: a } = o;
  return { fontFamily: o.fontFamily, fontSize: o.fontSize, fontSizeLG: o.fontSize, lineHeight: o.lineHeight, borderRadius: a.radius };
}, Xt = { ...Nt(Mt), primary: { ...Qt }, default: { ...Zt }, link: { ...Jt }, segmented: { default: { ...Pt }, ...qt() } }, L = q.colors, Lt = { rail: { colors: { background: { default: L.secondary.mediumGrey.mediumGrey200, hover: L.secondary.mediumGrey.mediumGrey200 } }, size: 4 }, handle: { colors: { default: L.primary.blue400 }, size: 14, border: 2 }, track: { colors: { background: { default: L.primary.blue400, hover: L.primary.blue400 } } }, dot: { colors: { border: { default: L.secondary.mediumGrey.mediumGrey200, active: L.primary.blue400 } }, size: 8 } }, Ht = { handle: { colors: { background: { hover: L.primary.blue100, focus: L.primary.blue400 }, border: { default: L.primary.blue400, disabled: L.secondary.mediumGrey.mediumGrey200 } } }, dot: { colors: { border: { default: L.secondary.mediumGrey.mediumGrey200 } } }, startIcon: { colors: { default: L.neutral.typography.black60 } }, endIcon: { colors: { default: L.neutral.typography.black60 } } }, Ot = (e) => ({ railBg: e.rail.colors.background.default, railHoverBg: e.rail.colors.background.hover, railSize: e.rail.size, handleColor: e.handle.colors.default, handleSize: e.handle.size, handleSizeHover: e.handle.size, handleLineWidth: e.handle.border, handleLineWidthHover: e.handle.border, trackBg: e.track.colors.background.default, trackHoverBg: e.track.colors.background.hover, dotActiveBorderColor: e.dot.colors.border.active, dotSize: e.dot.size, dotBorderColor: e.dot.colors.border.default }), Dt = { ...Ht, ...Ot(Lt) }, { colors: Ve, border: Rr } = q, jt = { colors: { background: { default: Ve.neutral.background }, border: { default: Ve.secondary.mediumGrey.mediumGrey300 } }, border: { radius: Rr.radius.s } }, $t = { colors: { background: { active: Ve.primary.blue100 }, border: { active: Ve.primary.blue400 } }, border: { radius: Rr.radius.s } }, _t = (o) => {
  var { colors: o, border: a } = o;
  return { colorFillAlter: o.background.default, colorBorder: o.border.default, borderRadius: a.radius };
}, ea = { ..._t(jt), ...$t }, { colors: ra, border: cr } = q, oa = { colors: { border: { none: "none", default: ra.neutral.typography.black60 } }, border: { radius: { default: cr.radius.square, rounded: cr.radius.s } } }, ta = { ...oa }, { colors: N, typography: Ue } = q, aa = { itemList: { colors: { background: { default: N.neutral.typography.white, disabled: N.primary.blue100 }, border: { default: N.neutral.typography.white, disabled: N.primary.blue100, hover: N.primary.blue400 } } }, title: { typography: { fontWeight: Ue.boldFontWeight, fontSize: Ue.fontSize5 }, colors: { default: N.primary.blue600, disabled: N.primary.blue200 } }, description: { typography: { fontWeight: Ue.regularFontWeight, fontSize: Ue.fontSize5 }, colors: { default: N.primary.blue600, disabled: N.primary.blue200 } }, collexp: { colors: { default: N.primary.blue600, disabled: N.primary.blue200, hover: N.primary.blue400 } }, rafter: { colors: { default: N.neutral.gray.gray400, disabled: N.primary.blue200, hover: N.primary.blue400 } } }, na = { ...aa }, { colors: oe, typography: Ye, border: Br, spacing: Ae } = q, ia = { card: { colors: { background: { default: oe.neutral.gray.gray100 }, border: { default: oe.neutral.gray.gray300, active: oe.primary.blue400 } }, border: { radius: Br.radius.xxs }, padding: Ae.xs + "px " + Ae.s + "px" }, typography: { fontFamily: Ye.fontFamily, fontSize: { s: Ye.fontSize5, m: Ye.fontSize5, l: Ye.fontSize5 }, colors: { disabled: oe.neutral.gray.gray300, active: oe.primary.blue600, hover: oe.primary.blue400, selected: oe.primary.blue400 } } }, la = { extraContent: { colors: { border: { default: oe.neutral.gray.gray300 }, typography: { default: oe.neutral.typography.black } }, border: { radius: Br.radius.xxs }, padding: Ae.xxs + "px " + Ae.s + "px", margin: { vertical: Ae.xxs, horizontal: Ae.s } } }, da = (o) => {
  var { card: o, typography: a } = o;
  return { fontFamily: a.fontFamily, titleFontSizeSM: a.fontSize.s, titleFontSize: a.fontSize.m, titleFontSizeLG: a.fontSize.l, cardBg: o.colors.background.default, colorBorderSecondary: o.colors.border.default, inkBarColor: o.colors.border.active, cardPadding: o.padding, cardPaddingLG: o.padding, cardPaddingSM: o.padding, borderRadius: o.border.radius, colorTextDisabled: a.colors.disabled, itemActiveColor: a.colors.active, itemHoverColor: a.colors.hover, itemSelectedColor: a.colors.selected };
}, ca = { ...da(ia), ...la }, { colors: z, typography: We } = q, sa = { colors: { background: { hover: z.primary.blue100 } }, typography: { fontFamily: We.fontFamily, fontSize: We.fontSize5 }, icon: { fontSize: We.fontSize5 } }, pa = { checkbox: { colors: { background: { default: z.neutral.typography.white, disabled: z.neutral.typography.white, checked: { default: z.primary.blue400, hover: z.primary.blue500 } }, border: { default: z.neutral.typography.black60, disabled: z.neutral.typography.black60, hover: z.primary.blue400, checked: { default: z.primary.blue400, hover: z.primary.blue500 } }, icon: { default: z.neutral.typography.black60, disabled: z.neutral.typography.black60, checked: z.neutral.typography.white } } }, treenode: { typography: { fontWeight: We.regularFontWeight }, colors: { typography: { hover: z.primary.blue400, disabled: z.neutral.typography.black60 }, background: { hover: z.primary.blue100, selected: z.primary.blue100, indent: z.neutral.typography.white } } } }, ma = (o) => {
  var { colors: o, typography: a, icon: i } = o;
  return { fontFamily: a.fontFamily, fontSize: a.fontSize, fontSizeIcon: i.fontSize, controlItemBgHover: o.background.hover };
}, ua = { ...ma(sa), ...pa }, Oe = { general: q, components: { Button: Xt, Rate: pt, Tooltip: ht, Tag: xt, InputWrapper: vt, Input: Ft, Slider: Dt, Avatar: It, AvatarGroup: Tt, Badge: zt, ItemCard: Vt, Collapse: ea, Image: ta, ItemList: na, Tabs: ca, Tree: ua } }, ga = { theme: Oe, overrideTheme: () => {
} }, Ur = fr(ga), Z = () => {
  var e = t.useContext(Ur);
  if (e === void 0)
    throw new Error("You need to encapsulate component inside a KitApp, useKitTheme must be inside a context");
  return e;
}, ha = ({ children: e }) => {
  var o = ya();
  return t.createElement(Ur.Provider, { value: o }, e);
}, ya = () => {
  const [e, o] = ue(Oe);
  return { theme: e, overrideTheme: (a) => {
    a = He(Oe, a), o(a);
  } };
}, Fl = ({ children: e, customTheme: o }) => t.createElement(ha, null, t.createElement(ba, { customTheme: o }, e)), ba = ({ children: e, customTheme: o }) => {
  var { theme: a, overrideTheme: i } = Z(), a = He(r, a), i = (o !== void 0 && i(o), He(a, o));
  return t.createElement(kr, { theme: i }, t.createElement(Do, null, t.createElement(Gr, null), t.createElement(Lo, null), t.createElement($o, null), t.createElement(et, null), t.createElement(ot, null), t.createElement(at, null), e));
}, fa = G(xr)`
    &:not(.ant-avatar-image) {
        ${({ $color: e, $theme: o }) => {
  switch (e) {
    case "default":
      return F`
                        background: ${o.colors.background.default};
                        color: ${o.colors.typography.default};
                    `;
    case "blueInvert":
      return F`
                        background: ${o.colors.background.blueInvert};
                        color: ${o.colors.typography.blueInvert};
                    `;
  }
}}
    }
`, Yr = ({ color: e = "default", ...o }) => {
  var a = Z().theme;
  return t.createElement(fa, { $theme: a.components.Avatar, $color: e, ...o });
}, ka = (Yr.displayName = "KitAvatar", (e) => {
  var o = Z().theme, o = o.components.AvatarGroup;
  return t.createElement(xr.Group, { maxStyle: { background: o.colors.background.default, color: o.colors.typography.default, cursor: e.maxPopoverTrigger === "click" ? "pointer" : "initial" }, ...e });
}), sr = Yr, xa = (sr.displayName = "KitAvatar", sr.Group = ka, G(Dr)`
    height: 16px;
    min-width: 16px;
    line-height: 16px;
    font-size: ${r.typography.fontSize7};

    .ant-badge-count {
        ${({ $countColor: e, $theme: o }) => {
  switch (e) {
    case "default":
      return F`
                        background: ${o.colors.background.default};
                        color: ${o.colors.typography.default};
                    `;
    case "green":
      return F`
                        background: ${o.colors.background.green};
                        color: ${o.colors.typography.green};
                    `;
    case "blue":
      return F`
                        background: ${o.colors.background.blue};
                        color: ${o.colors.typography.blue};
                    `;
    case "blueInvert":
      return F`
                        background: ${o.colors.background.blueInvert};
                        color: ${o.colors.typography.blueInvert};
                    `;
    case "gray":
      return F`
                        background: ${o.colors.background.gray};
                        color: ${o.colors.typography.gray};
                    `;
  }
}}
    }

    &.ant-badge {
        .ant-badge-status-processing {
            &.ant-scroll-number {
                position: absolute;
            }

            &::after {
                animation-name: none;
                content: initial;
            }

            &.ant-badge-status-dot {
                &::after {
                    animation-name: none;
                    content: initial;
                }
            }
        }
    }
`), Aa = ({ countColor: e = "default", ...o }) => {
  var a = Z().theme;
  return t.createElement(xa, { $theme: a.components.Badge, $countColor: e, ...o });
}, ne = (Aa.displayName = "KitBadge", (e) => t.createElement(jr, { ...e })), va = (ne.displayName = "KitTooltip", k.div`
    width: ${({ $column: e, $theme: o }) => e ? o.colorBar.thickness + "px" : "auto"};
    height: ${({ $column: e, $theme: o }) => e ? "auto" : o.colorBar.thickness + "px"};
    border-radius: ${({ $theme: e }) => e.colorBar.border.radius}px;
    display: flex;
    flex-direction: ${({ $column: e }) => e ? "column" : "row"};
    overflow: hidden;

    > div {
        flex: 1 1 auto;
    }
`), Ca = (e) => ({ background: e.color ?? "transparent" }), wa = (e) => {
  var a;
  var o = Z().theme;
  return t.createElement(va, { $theme: o.components.ItemCard, $column: e.vertical ?? !1, className: e.className }, (a = e.colors) == null ? void 0 : a.map((i, l) => t.createElement(ne, { key: i.label + "_" + l, title: i.label, placement: e.vertical ? "right" : "top" }, t.createElement("div", { style: Ca(i) }))));
}, Sa = G($r)`
    height: 40px;
    min-width: 40px;
    box-shadow: none;
    color: ${({ $theme: e }) => e.colors.typography.default};
    background-color: ${({ $theme: e }) => e.colors.background.default};
    border-color: ${({ $theme: e }) => e.colors.border.default};
    font-weight: ${({ $theme: e }) => e.typography.fontWeight};

    &.ant-btn .ant-btn-icon .anticon {
        font-size: ${({ $theme: e, $iconSize: o }) => o === void 0 ? e.typography.iconSize.m : e.typography.iconSize[o]}px;
    }

    &.ant-btn-primary.ant-btn-background-ghost {
        color: ${({ $theme: e }) => e.colors.typography.ghost};
        background-color: ${({ $theme: e }) => e.colors.background.ghost};
    }

    + .kit-btn-segmented-actived-icon {
        position: absolute;
        z-index: 2;
        bottom: 6px;
        right: 5px;
        font-size: 12px;
        color: ${({ $theme: e }) => e.colors.typography.iconCheck};
    }

    &.kit-btn-segmented {
        border-radius: 10px;

        &.kit-btn-segmented-actived:not(.ant-btn-dangerous) {
            color: ${({ $theme: e }) => e.colors.typography.active};
            background-color: ${({ $theme: e }) => e.colors.background.active};
            border-color: ${({ $theme: e }) => e.colors.border.active};
            z-index: 2;

            &:not(.ant-btn-link):not(.ant-btn-loading):not(.ant-btn-dangerous):not(:disabled) {
                &:hover {
                    color: ${({ $theme: e }) => e.colors.typography.active};
                    background-color: ${({ $theme: e }) => e.colors.background.active};
                }
            }
        }

        &.kit-btn-segmented-actived.ant-btn-dangerous {
            color: ${({ $theme: e }) => e.colors.typography.danger.default};
            background-color: ${({ $theme: e }) => e.colors.background.danger.active};
            border-color: ${({ $theme: e }) => e.colors.border.danger.active};

            &:not(.ant-btn-loading):not(:disabled) {
                &:hover {
                    background-color: ${({ $theme: e }) => e.colors.background.danger.active};
                }
            }
        }

        &:disabled {
            cursor: not-allowed;
        }
    }

    &.ant-btn-compact-item {
        border-radius: 0;

        &.ant-btn-compact-first-item {
            border-radius: ${({ $theme: e }) => e.compact.border.radius}px 0 0
                ${({ $theme: e }) => e.compact.border.radius}px;
        }

        &.ant-btn-compact-last-item {
            border-radius: 0 ${({ $theme: e }) => e.compact.border.radius}px
                ${({ $theme: e }) => e.compact.border.radius}px 0;
        }

        &.ant-btn-compact-first-item.ant-btn-compact-last-item {
            border-radius: ${({ $theme: e }) => e.compact.border.radius}px;
        }
    }

    &.ant-btn-compact-vertical-item {
        border-radius: 0;

        &.ant-btn-compact-vertical-first-item {
            border-radius: ${({ $theme: e }) => e.compact.border.radius}px
                ${({ $theme: e }) => e.compact.border.radius}px 0 0;
        }

        &.ant-btn-compact-vertical-last-item {
            border-radius: 0 0 ${({ $theme: e }) => e.compact.border.radius}px
                ${({ $theme: e }) => e.compact.border.radius}px;
        }

        &.ant-btn-compact-first-item.ant-btn-compact-last-item {
            border-radius: ${({ $theme: e }) => e.compact.border.radius}px;
        }
    }

    &:disabled,
    &.ant-btn-loading:not(:disabled) {
        color: ${({ $theme: e }) => e.colors.typography.disabled};
        background-color: ${({ $theme: e }) => e.colors.background.disabled};
        border-color: ${({ $theme: e }) => e.colors.border.disabled};
        opacity: initial;

        &:hover {
            color: ${({ $theme: e }) => e.colors.typography.disabled};
            background-color: ${({ $theme: e }) => e.colors.background.disabled};
            border-color: ${({ $theme: e }) => e.colors.border.disabled};
        }
    }

    &:not(.ant-btn-link):not(.ant-btn-loading):not(.ant-btn-dangerous):not(:disabled) {
        &:hover {
            color: ${({ $theme: e }) => e.colors.typography.hover};
            background-color: ${({ $theme: e }) => e.colors.background.hover};
            border-color: ${({ $theme: e }) => e.colors.border.hover};
            border-style: solid;
        }

        &:focus {
            color: ${({ $theme: e }) => e.colors.typography.focus};
            background-color: ${({ $theme: e }) => e.colors.background.focus};
            border-color: ${({ $theme: e }) => e.colors.border.focus};
            border-style: dashed;
        }

        &:active {
            color: ${({ $theme: e }) => e.colors.typography.active};
            background-color: ${({ $theme: e }) => e.colors.background.active};
            border-color: ${({ $theme: e }) => e.colors.border.active};
            border-style: solid;
        }
    }

    &.ant-btn-link {
        height: 22px;
        padding: 0px;
        border: none;
        border-radius: 0px;
        border-bottom: 1px solid ${({ $theme: e }) => e.colors.border.default};

        &:disabled {
            border-bottom: none;
        }

        &:not(.ant-btn-loading):not(:disabled) {
            &:hover {
                color: ${({ $theme: e }) => e.colors.typography.default};
                border-bottom: 1px dashed ${({ $theme: e }) => e.colors.border.hover};
            }

            &:focus {
                border-bottom: 1px dashed ${({ $theme: e }) => e.colors.border.focus};
            }

            &:active {
                border-bottom: 1px dashed ${({ $theme: e }) => e.colors.border.active};
            }
        }
    }

    &.ant-btn-dangerous {
        box-shadow: none;
        color: ${({ $theme: e }) => e.colors.typography.danger.default};
        background-color: ${({ $theme: e }) => e.colors.background.danger.default};
        border-color: ${({ $theme: e }) => e.colors.border.danger.default};

        &:disabled {
            color: ${({ $theme: e }) => e.colors.typography.danger.disabled};
            background-color: ${({ $theme: e }) => e.colors.background.danger.disabled};
            border-color: ${({ $theme: e }) => e.colors.border.danger.disabled};
            opacity: initial;
        }

        &:not(.ant-btn-loading):not(:disabled) {
            &:hover {
                color: ${({ $theme: e }) => e.colors.typography.danger.default};
                background-color: ${({ $theme: e }) => e.colors.background.danger.hover};
                border-color: ${({ $theme: e }) => e.colors.border.danger.hover};

                &.ant-btn-link {
                    border-width: 0 0 1px 0;
                }
            }

            &:focus {
                color: ${({ $theme: e }) => e.colors.typography.danger.default};
                background-color: ${({ $theme: e }) => e.colors.background.danger.focus};
                border-color: ${({ $theme: e }) => e.colors.border.danger.focus};
                border-style: dashed;

                &.ant-btn-link {
                    border-width: 0 0 1px 0;
                }
            }

            &:active {
                color: ${({ $theme: e }) => e.colors.typography.danger.default};
                background-color: ${({ $theme: e }) => e.colors.background.danger.active};
                border-color: ${({ $theme: e }) => e.colors.border.danger.active};

                &.ant-btn-link {
                    border-width: 0 0 1px 0;
                }
            }
        }
    }
`, Ea = ({ iconSize: e, primaryModal: o, type: a, segmentedChecked: i, segmentedActived: l, segmentedColor: c, className: s, wrapperClassName: p, wrapperStyle: h, ...m }, x) => {
  var K = Z().theme;
  const S = K.components.Button;
  return t.createElement("div", { className: p ?? "", style: { position: "relative", display: "inline-block", ...h } }, t.createElement(Sa, { $theme: (() => {
    switch (a) {
      case "primary":
        return S.primary;
      case "link":
        return S.link;
      case "segmented":
        return c ? S.segmented[c] : S.segmented.default;
      default:
        return S.default;
    }
  })(), $iconSize: e, ...m, className: (s || "") + (a === "segmented" ? " kit-btn-segmented" : "") + (l ? " kit-btn-segmented-actived" : ""), ghost: o, type: (() => {
    if (o)
      return "primary";
    switch (a) {
      case "segmented":
      default:
        return "default";
      case "primary":
      case "link":
      case "default":
        return a;
    }
  })(), ref: x }), a === "segmented" && i && t.createElement(So, { className: "kit-btn-segmented-actived-icon" }));
}, te = t.forwardRef(Ea), Fa = (te.displayName = "KitButton", [1, 2, 3, 4, 5, 6, 7]), ie = F`
    &.ant-typography {
        font-family: 'Inter';
        line-height: 1.2;
        font-weight: ${r.typography.defaultFontWeight};
        color: ${r.color.neutral.typography.black};
    }
`, le = { large: 5, medium: 6, small: 7 }, ar = (e) => Ge(e.className, { "ant-typography-regular": e.weight === "regular", "ant-typography-medium": e.weight === "medium", "ant-typography-bold": e.weight === "bold" }), Ka = (F`
    ${ie};
    font-size: ${(e) => e.size ? r.typography["fontSize" + le[e.size]] : r.typography.fontSize6}px;
    line-height: ${(e) => e.size ? r.typography["lineHeight" + le[e.size]] : r.typography.lineHeight6};
`, G.h1`
    ${ie};
    &.ant-typography {
        font-size: ${$.fontSize1}px;
        line-height: ${$.lineHeight1};
    }
`), Ia = G.h1`
    ${ie};
    &.ant-typography {
        font-size: ${$.fontSize2}px;
        line-height: ${$.lineHeight2};
    }
`, Ga = G.h3`
    ${ie};
    &.ant-typography {
        font-size: ${$.fontSize3}px;
        line-height: ${$.lineHeight3};
    }
`, Ta = G.h4`
    ${ie};
    &.ant-typography {
        font-size: ${$.fontSize4}px;
        line-height: ${$.lineHeight4};
    }
`, Ra = G.h5`
    ${ie};
    &.ant-typography {
        font-size: ${r.typography.fontSize5}px;
        line-height: ${r.typography.lineHeight5};
        font-weight: ${(e) => e.regular ? r.typography.regularFontWeight : r.typography.mediumfontWeight};
    }
`, Ba = G.h6`
    ${ie};
    &.ant-typography {
        font-size: ${r.typography.fontSize6}px;
        line-height: ${r.typography.lineHeight6};
        font-weight: ${(e) => e.regular ? r.typography.regularFontWeight : r.typography.mediumfontWeight};
    }
`, Ua = G.p`
    ${ie};
    &.ant-typography {
        font-size: ${r.typography.fontSize7}px;
        line-height: ${r.typography.lineHeight7};
        font-weight: ${(e) => e.regular ? r.typography.regularFontWeight : r.typography.mediumfontWeight};
    }
`, pr = [Ka, Ia, Ga, Ta, Ra, Ba, Ua], Ya = u.forwardRef((e, o) => {
  const { level: a = 1, ...i } = e;
  let l = pr[0];
  return Fa.includes(a) && (l = pr[a - 1]), u.createElement(Ro, { ref: o, ...i, component: l });
}), za = G($e.Text)`
    &.ant-typography {
        font-family: 'Inter';
        line-height: 1.2;
        font-weight: ${r.typography.defaultFontWeight};
        color: ${r.color.neutral.typography.black};
    }

    &.ant-typography-regular {
        font-weight: ${r.typography.regularFontWeight};
    }
    &.ant-typography-medium {
        font-weight: ${r.typography.mediumfontWeight};
    }
    &.ant-typography-bold {
        font-weight: ${r.typography.boldFontWeight};
    }

    font-size: ${(e) => e.size ? r.typography["fontSize" + le[e.size]] : r.typography.fontSize6}px;
    line-height: ${(e) => e.size ? r.typography["lineHeight" + le[e.size]] : r.typography.lineHeight6};
`, Wa = t.forwardRef((e, o) => t.createElement(za, { ref: o, ...e, className: ar(e) })), Va = k($e.Link)`
    &.ant-typography {
        font-family: 'Inter';
        line-height: 1.2;
        font-weight: ${r.typography.defaultFontWeight};
        color: ${r.color.neutral.typography.black};
    }

    &.ant-typography-regular {
        font-weight: ${r.typography.regularFontWeight};
    }
    &.ant-typography-medium {
        font-weight: ${r.typography.mediumfontWeight};
    }
    &.ant-typography-bold {
        font-weight: ${r.typography.boldFontWeight};
    }

    font-size: ${(e) => e.size ? r.typography["fontSize" + le[e.size]] : r.typography.fontSize6}px;
    line-height: ${(e) => e.size ? r.typography["lineHeight" + le[e.size]] : r.typography.lineHeight6};
`, Ma = t.forwardRef((e, o) => t.createElement(Va, { ref: o, ...e, className: ar(e) })), Qa = k($e.Paragraph)`
    &.ant-typography {
        font-family: 'Inter';
        line-height: 1.2;
        font-weight: ${r.typography.defaultFontWeight};
        color: ${r.color.neutral.typography.black};
    }

    &.ant-typography-regular {
        font-weight: ${r.typography.regularFontWeight};
    }
    &.ant-typography-medium {
        font-weight: ${r.typography.mediumfontWeight};
    }
    &.ant-typography-bold {
        font-weight: ${r.typography.boldFontWeight};
    }

    font-size: ${(e) => e.size ? r.typography["fontSize" + le[e.size]] : r.typography.fontSize6}px;
    line-height: ${(e) => e.size ? r.typography["lineHeight" + le[e.size]] : r.typography.lineHeight6};
`, Za = t.forwardRef((e, o) => t.createElement(Qa, { ref: o, ...e, className: ar(e) })), M = { Title: Ya, Text: Wa, Link: Ma, Paragraph: Za, displayName: "KitTypography" }, E = { fontWeight: { default: r.typography.mediumfontWeight, checked: r.typography.boldFontWeight }, borderWidth: "1px", color: { default: r.color.neutral.typography.black, disabled: r.color.secondary.mediumGrey.mediumGrey400, danger: { default: r.color.neutral.typography.black, disabled: r.color.secondary.red.red200 } }, backgroundColor: { default: r.color.primary.blue400, hover: r.color.primary.blue500, active: r.color.primary.blue600, focus: r.color.primary.blue400, disabled: r.color.secondary.mediumGrey.mediumGrey100, danger: { default: r.color.secondary.red.red200, hover: r.color.secondary.red.red500, active: r.color.secondary.red.red600, focus: r.color.secondary.red.red400, disabled: r.color.secondary.red.red400 } }, borderColor: { default: r.color.primary.blue400, hover: r.color.primary.blue500, active: r.color.primary.blue600, focus: r.color.primary.blue400, disabled: r.color.secondary.mediumGrey.mediumGrey200, danger: { default: r.color.secondary.red.red400, hover: r.color.secondary.red.red500, active: r.color.secondary.red.red600, focus: r.color.secondary.red.red400, disabled: r.color.secondary.red.red200 } } }, Se = r.color, Ja = k(_r)`
    font-weight: ${E.fontWeight.default};

    &:not(.ant-checkbox-wrapper-danger) {
        .ant-checkbox-disabled + span {
            color: ${E.color.disabled};
        }
    }

    .ant-checkbox-disabled {
        color: ${E.color.disabled};

        .ant-checkbox-inner {
            background-color: ${E.backgroundColor.disabled};
            border: ${E.borderWidth} solid ${E.borderColor.disabled};
        }

        .ant-checkbox-inner:after {
            border-color: ${E.borderColor.disabled};
        }
    }

    &.ant-checkbox-wrapper-danger .ant-checkbox-disabled .ant-checkbox-inner:after {
        border-color: ${E.borderColor.danger.default};
    }

    .ant-checkbox-indeterminate {
        &:not(.ant-checkbox-disabled) .ant-checkbox-inner {
            background-color: ${E.backgroundColor.default};
            border-color: ${E.backgroundColor.default};
        }

        .ant-checkbox-inner:after {
            width: 8px;
            height: 0px;
            border: ${E.borderWidth} solid white;
            transform: translate('-4px', '-1px');
        }
        &.ant-checkbox-disabled .ant-checkbox-inner:after {
            border-color: ${E.borderColor.disabled};
        }
    }

    &.ant-checkbox-wrapper-danger {
        .ant-checkbox {
            &.ant-checkbox-disabled + span {
                color: ${E.color.danger.disabled};
            }

            .ant-checkbox-inner {
                background-color: ${E.backgroundColor.danger.default};
                border: ${E.borderWidth} solid ${E.borderColor.danger.default};
            }
        }
    }

    .ant-checkbox-checked + span {
        font-weight: ${E.fontWeight.default};
    }

    .ant-checkbox-checked:after {
        border: none;
    }

    &.ant-checkbox-wrapper-danger .ant-checkbox-checked:after {
        border: none;
    }

    // Hover
    &:not(.ant-checkbox-wrapper-disabled):hover,
    &:not(.ant-checkbox-wrapper-disabled) .ant-checkbox:hover {
        .ant-checkbox-inner {
            border-color: ${E.borderColor.hover};
        }
    }

    &.ant-checkbox-wrapper-danger:not(.ant-checkbox-wrapper-disabled):hover,
    &.ant-checkbox-wrapper-danger:not(.ant-checkbox-wrapper-disabled):hover .ant-checkbox-checked,
    &.ant-checkbox-wrapper-danger:not(.ant-checkbox-wrapper-disabled) .ant-checkbox:hover {
        .ant-checkbox-inner {
            border-color: ${E.borderColor.danger.hover};
            background-color: ${E.backgroundColor.danger.hover};
        }
    }

    &.ant-checkbox-wrapper .ant-checkbox-input:focus + .ant-checkbox-inner {
        border: ${E.borderWidth} dashed ${E.borderColor.focus};
    }
    &.ant-checkbox-wrapper-danger .ant-checkbox-input:focus + .ant-checkbox-inner {
        border: ${E.borderWidth} dashed ${E.borderColor.danger.focus};
    }

    // Focus

    &.ant-checkbox-wrapper:not(.ant-checkbox-wrapper-disabled):not(.ant-checkbox-wrapper-danger):hover
        .ant-checkbox:focus-within
        .ant-checkbox-inner {
        border: 1px dashed ${Se.primary.blue400};
        background-color: white;
        &:after {
            border-color: ${Se.primary.blue300};
            border-top: 0;
            border-inline-start: 0;
        }
    }

    &.ant-checkbox-wrapper:not(.ant-checkbox-wrapper-disabled):not(.ant-checkbox-wrapper-danger)
        .ant-checkbox:focus-within
        .ant-checkbox-inner {
        background-color: white;
        &:after {
            border-color: ${Se.primary.blue400};
            border-top: 0;
            border-inline-start: 0;
        }
    }

    &.ant-checkbox-wrapper:not(.ant-checkbox-wrapper-disabled):not(.ant-checkbox-wrapper-danger)
        .ant-checkbox:focus-within
        .ant-checkbox-inner {
        background-color: white;
        &:after {
            border-color: ${Se.primary.blue400};
            border-top: 0;
            border-inline-start: 0;
            border-width: 2px;
        }
    }

    &.ant-checkbox-wrapper-danger:not(.ant-checkbox-wrapper-disabled) .ant-checkbox:focus-within {
        .ant-checkbox-inner {
            background-color: white;
            &:after {
                border-color: ${Se.secondary.red.red400};
                border-top: 0;
                border-inline-start: 0;
                border-width: 2px;
            }
        }
    }
`, zr = t.forwardRef(({ danger: e, ...o }, a) => (e = e ? (o.className || "") + " ant-checkbox-wrapper-danger" : o.className, t.createElement(Ja, { ...o, ref: a, className: e }))), Pa = ({ defaultValue: w, children: o, options: a = [], prefixCls: ce, className: l, rootClassName: c, style: s, onChange: p, ...h }, m) => {
  var { getPrefixCls: H, direction: O } = u.useContext(Ir);
  const [x, K] = u.useState(h.value || w || []), [S, A] = u.useState([]), T = (u.useEffect(() => {
    "value" in h && K(h.value || []);
  }, [h]), () => a.map((g) => typeof g == "string" || typeof g == "number" ? { label: g, value: g } : g)), C = H("checkbox", ce), U = C + "-group";
  var [w, H] = Yo(C), ce = Bo(h, ["value", "disabled"]), ee = (a && 0 < a.length && (o = T().map((g) => u.createElement(zr, { prefixCls: C, key: g.value.toString(), danger: ("danger" in g ? g : h).danger, disabled: ("disabled" in g ? g : h).disabled, value: g.value, checked: x.includes(g.value), onChange: g.onChange, className: U + "-item", style: g.style }, g.label))), { toggleOption: (g) => {
    var _ = x.indexOf(g.value), D = [...x];
    _ === -1 ? D.push(g.value) : D.splice(_, 1), "value" in h || K(D);
    const R = T();
    p == null || p(D.filter((se) => S.includes(se)).sort((se, Te) => R.findIndex((he) => he.value === se) - R.findIndex((he) => he.value === Te)));
  }, value: x, disabled: h.disabled, name: h.name, registerValue: (g) => {
    A((_) => [..._, g]);
  }, cancelValue: (g) => {
    A((_) => _.filter((D) => D !== g));
  } }), O = Ge(U, { [U + "-rtl"]: O === "rtl" }, l, c, H);
  return w(u.createElement("div", { className: O, style: s, ...ce, ref: m }, u.createElement(Uo.Provider, { value: ee }, o)));
}, qa = u.forwardRef(Pa), Na = u.memo(qa), ve = zr, de = (ve.displayName = "KitCheckbox", ve.Group = Na, ({ label: e, helper: o, disabled: a, status: i, children: l }) => {
  var c = Z().theme;
  const { colors: s, gap: p } = c.components.InputWrapper;
  return c = (() => {
    if (a)
      return s.typography.disabled;
    switch (i) {
      case "warning":
        return s.typography.warning;
      case "error":
        return s.typography.error;
      default:
        return s.typography.default;
    }
  })(), t.createElement("div", { className: "kit-input-wrapper" }, e && t.createElement("div", { className: "kit-input-label", style: { marginBottom: p.label } }, t.createElement(M.Text, { size: "large", weight: "medium", style: { color: c } }, e)), l, o && t.createElement("div", { className: "kit-input-helper", style: { marginTop: p.helper } }, t.createElement(M.Text, { size: "small", weight: "regular", style: { color: c } }, "* ", o)));
}), Xa = G(_e)`
    &.ant-input,
    .ant-input {
        height: 40px;
        padding: 0px 10px;
        font-weight: ${({ $theme: e }) => e.typography.content.fontWeight};

        &::placeholder {
            font-weight: ${({ $theme: e }) => e.typography.placeholder.fontWeight};
        }
    }

    &.ant-input-affix-wrapper {
        padding: 0px 12px;

        .ant-input-prefix {
            margin-inline-end: 10px;
            color: ${({ $theme: e }) => e.colors.prefix.default};
        }

        .ant-input-suffix {
            margin-inline-start: 10px;
            color: ${({ $theme: e }) => e.colors.suffix.default};

            .ant-input-clear-icon {
                color: ${({ $theme: e }) => e.colors.clearIcon.default};
            }
        }

        .ant-input-suffix .ant-input-show-count-suffix {
            color: ${({ $theme: e }) => e.colors.showCount.default};
            font-size: ${({ $theme: e }) => e.typography.showCount.fontSize}px;
            font-weight: ${({ $theme: e }) => e.typography.showCount.fontWeight};
        }

        .ant-input {
            font-weight: ${({ $theme: e }) => e.typography.content.fontWeight};
            height: 38px;

            &::placeholder {
                font-weight: ${({ $theme: e }) => e.typography.placeholder.fontWeight};
            }
        }

        &.ant-input-affix-wrapper-disabled {
            border-color: ${({ $theme: e }) => e.colors.border.disabled};

            .ant-input-prefix {
                color: ${({ $theme: e }) => e.colors.prefix.disabled};
            }

            .ant-input-suffix {
                color: ${({ $theme: e }) => e.colors.suffix.disabled};

                .ant-input-show-count-suffix {
                    color: ${({ $theme: e }) => e.colors.showCount.disabled};
                }

                .ant-input-clear-icon {
                    color: ${({ $theme: e }) => e.colors.clearIcon.disabled};
                }
            }
        }

        &.ant-input-affix-wrapper-status-error:not(.ant-input-disabled) {
            .ant-input-prefix {
                color: ${({ $theme: e }) => e.colors.prefix.error};
            }

            .ant-input-suffix {
                color: ${({ $theme: e }) => e.colors.suffix.error};
            }

            .ant-input-suffix {
                .ant-input-show-count-suffix {
                    color: ${({ $theme: e }) => e.colors.showCount.error};
                }

                .ant-input-clear-icon {
                    color: ${({ $theme: e }) => e.colors.clearIcon.error};
                }
            }
        }

        &.ant-input-affix-wrapper-status-warning:not(.ant-input-disabled) {
            .ant-input-prefix {
                color: ${({ $theme: e }) => e.colors.prefix.warning};
            }

            .ant-input-suffix {
                color: ${({ $theme: e }) => e.colors.suffix.warning};
            }

            .ant-input-suffix {
                .ant-input-show-count-suffix {
                    color: ${({ $theme: e }) => e.colors.showCount.warning};
                }

                .ant-input-clear-icon {
                    color: ${({ $theme: e }) => e.colors.clearIcon.warning};
                }
            }
        }
    }

    &.ant-input-disabled,
    .ant-input-disabled {
        border-color: ${({ $theme: e }) => e.colors.border.disabled};
        color: ${({ $theme: e }) => e.colors.typography.content.disabled};

        &:hover {
            border-color: ${({ $theme: e }) => e.colors.border.disabled};
        }

        &::placeholder {
            color: ${({ $theme: e }) => e.colors.typography.placeholder.disabled};
        }
    }

    &.ant-input-status-error,
    &.ant-input-affix-wrapper.ant-input-affix-wrapper-status-error,
    &.ant-input-affix-wrapper.ant-input-affix-wrapper-status-error .ant-input {
        background-color: ${({ $theme: e }) => e.colors.background.error};
        color: ${({ $theme: e }) => e.colors.typography.content.error};

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${({ $theme: e }) => e.colors.border.error};
            box-shadow: none;

            &:hover,
            &:focus {
                border-color: ${({ $theme: e }) => e.colors.border.error};
            }
        }

        &::placeholder {
            color: ${({ $theme: e }) => e.colors.typography.placeholder.error};
        }
    }

    &.ant-input-status-warning,
    &.ant-input-affix-wrapper.ant-input-affix-wrapper-status-warning,
    &.ant-input-affix-wrapper.ant-input-affix-wrapper-status-warning .ant-input {
        background-color: ${({ $theme: e }) => e.colors.background.warning};
        color: ${({ $theme: e }) => e.colors.typography.content.warning};

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${({ $theme: e }) => e.colors.border.warning};
            box-shadow: none;

            &:hover,
            &:focus {
                border-color: ${({ $theme: e }) => e.colors.border.warning};
            }
        }

        &::placeholder {
            color: ${({ $theme: e }) => e.colors.typography.placeholder.warning};
        }
    }

    &:focus-within,
    &.ant-input-affix-wrapper.ant-input-affix-wrapper-focused {
        border-style: dashed;
        box-shadow: none;

        &.ant-input::placeholder,
        .ant-input::placeholder {
            color: transparent;
        }
    }
`, La = t.forwardRef(({ label: e, helper: o, allowClear: a = !0, ...i }, l) => {
  var c = Z().theme;
  return t.createElement(de, { label: e, helper: o, disabled: i.disabled, status: i.status }, t.createElement(Xa, { ref: l, $theme: c.components.Input, ...i, allowClear: a ? { clearIcon: t.createElement(ge, null) } : void 0 }));
}), Ha = G(_e.TextArea)`
    &.ant-input,
    .ant-input {
        padding: 4px 10px;
        font-weight: ${r.typography.mediumfontWeight};

        &::placeholder {
            font-weight: ${r.typography.regularFontWeight};
        }
    }

    .ant-input-suffix {
        .ant-input-clear-icon {
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }
    }

    &.ant-input-affix-wrapper {
        &.ant-input-textarea-allow-clear {
            .ant-input {
                padding-inline-end: 24px;
            }
        }

        &.ant-input-textarea-show-count {
            .ant-input-data-count {
                bottom: -23px;
                color: ${r.color.secondary.mediumGrey.mediumGrey300};
                font-size: ${r.typography.fontSize7}px;
                font-weight: ${r.typography.regularFontWeight};
                line-height: ${r.typography.lineHeight7};
            }
        }

        &.ant-input-affix-wrapper-disabled {
            .ant-input-data-count {
                color: ${r.color.secondary.mediumGrey.mediumGrey300};
            }

            .ant-input-suffix {
                .ant-input-clear-icon {
                    color: ${r.color.secondary.mediumGrey.mediumGrey300};
                }
            }
        }

        &.ant-input-affix-wrapper-status-error {
            .ant-input-data-count {
                color: ${r.color.secondary.red.red300};
            }

            .ant-input-suffix {
                .ant-input-clear-icon {
                    color: ${r.color.secondary.red.red400};
                }
            }

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.red.red400};
            }
        }

        &.ant-input-affix-wrapper-status-warning {
            .ant-input-data-count {
                color: ${r.color.secondary.orange.orange300};
            }

            .ant-input-suffix {
                .ant-input-clear-icon {
                    color: ${r.color.secondary.orange.orange500};
                }
            }

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.orange.orange400};
            }
        }
    }

    &.ant-input-disabled,
    .ant-input-disabled {
        border-color: ${r.color.secondary.mediumGrey.mediumGrey200};
        color: ${r.color.secondary.mediumGrey.mediumGrey400};

        &:hover {
            border-color: ${r.color.secondary.mediumGrey.mediumGrey200};
        }

        &::placeholder {
            color: ${r.color.secondary.mediumGrey.mediumGrey400};
        }
    }

    &.ant-input-status-error,
    &.ant-input-affix-wrapper-status-error,
    .ant-input-status-error {
        background-color: ${r.color.secondary.red.red100};
        color: ${r.color.secondary.red.red400};

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${r.color.secondary.red.red400};
            box-shadow: none;

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.red.red400};
            }
        }

        &::placeholder {
            color: ${r.color.secondary.red.red400};
        }
    }

    &.ant-input-status-warning,
    &.ant-input-affix-wrapper-status-warning,
    .ant-input-status-warning {
        background-color: ${r.color.secondary.orange.orange100};
        color: ${r.color.secondary.orange.orange500};

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${r.color.secondary.orange.orange400};
            box-shadow: none;

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.orange.orange400};
            }
        }

        &::placeholder {
            color: ${r.color.secondary.orange.orange400};
        }
    }

    &:focus,
    &.ant-input-affix-wrapper-focused {
        border-style: dashed;
        box-shadow: none;

        .ant-input::placeholder {
            color: transparent;
        }
    }
`, Oa = ({ label: e, helper: o, allowClear: a = !0, ...i }) => t.createElement(de, { label: e, helper: o, disabled: i.disabled, status: i.status }, t.createElement(Ha, { ...i, allowClear: a ? { clearIcon: t.createElement(ge, null) } : void 0 })), Da = G(_e.Password)`
    &.ant-input-affix-wrapper {
        padding: 0px 12px;

        .ant-input {
            height: 38px;
            padding: 0px 10px 0px 0px;
            border-radius: 7px;
            font-weight: ${r.typography.mediumfontWeight};

            &::placeholder {
                font-weight: ${r.typography.regularFontWeight};
            }
        }

        .ant-input-suffix {
            .ant-input-password-icon {
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }

            .ant-input-show-count-suffix {
                color: ${r.color.secondary.mediumGrey.mediumGrey300};
                font-size: ${r.typography.fontSize7};
                font-weight: ${r.typography.regularFontWeight};
            }

            .ant-input-clear-icon {
                margin-top: 2px;
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }
        }

        .ant-input-prefix {
            margin-inline-end: 10px;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        &.ant-input-affix-wrapper-disabled {
            border-color: ${r.color.secondary.mediumGrey.mediumGrey200};

            .ant-input {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};

                &::placeholder {
                    color: ${r.color.secondary.mediumGrey.mediumGrey400};
                }
            }

            .ant-input-prefix,
            .ant-input-suffix,
            .ant-input-suffix .anticon {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }
        }

        &.ant-input-affix-wrapper-status-error,
        &.ant-input-affix-wrapper-status-error .ant-input {
            background-color: ${r.color.secondary.red.red100};
            color: ${r.color.secondary.red.red400};

            &:not(.ant-input-borderless):not(.ant-input-disabled) {
                border-color: ${r.color.secondary.red.red400};
                box-shadow: none;

                &:hover,
                &:focus {
                    border-color: ${r.color.secondary.red.red400};
                }
            }

            &::placeholder {
                color: ${r.color.secondary.red.red400};
            }

            .ant-input-prefix,
            .ant-input-suffix {
                .anticon {
                    color: ${r.color.secondary.red.red400};
                }
            }

            .ant-input-suffix {
                .ant-input-show-count-suffix {
                    color: ${r.color.secondary.red.red300};
                }

                .ant-input-clear-icon {
                    .anticon {
                        color: ${r.color.secondary.red.red400};
                    }
                }
            }
        }

        &.ant-input-affix-wrapper-status-warning,
        &.ant-input-affix-wrapper-status-warning .ant-input {
            background-color: ${r.color.secondary.orange.orange100};
            color: ${r.color.secondary.orange.orange500};

            &:not(.ant-input-borderless):not(.ant-input-disabled) {
                border-color: ${r.color.secondary.orange.orange400};
                box-shadow: none;

                &:hover,
                &:focus {
                    border-color: ${r.color.secondary.orange.orange400};
                }
            }

            &::placeholder {
                color: ${r.color.secondary.orange.orange400};
            }

            .ant-input-prefix,
            .ant-input-suffix {
                .anticon {
                    color: ${r.color.secondary.orange.orange400};
                }
            }

            .ant-input-suffix {
                .ant-input-show-count-suffix {
                    color: ${r.color.secondary.orange.orange300};
                }

                .ant-input-clear-icon {
                    .anticon {
                        color: ${r.color.secondary.orange.orange500};
                    }
                }
            }
        }

        &.ant-input-affix-wrapper-focused {
            border-style: dashed;
            box-shadow: none;

            .ant-input::placeholder {
                color: transparent;
            }
        }
    }
`, ja = ({ label: e, helper: o, allowClear: a = !0, ...i }) => t.createElement(de, { label: e, helper: o, disabled: i.disabled, status: i.status }, t.createElement(Da, { ...i, allowClear: a ? { clearIcon: t.createElement(ge, null) } : void 0 })), Ke = La, $a = (Ke.displayName = "KitInput", Ke.TextArea = Oa, Ke.Password = ja, G(eo)`
    font-weight: ${r.typography.regularFontWeight};
    height: 40px;
    line-height: 40px;

    .ant-input-number-input {
        font-weight: ${r.typography.mediumfontWeight};

        &::placeholder {
            font-weight: ${r.typography.regularFontWeight};
        }
    }

    &:not(.ant-input-number-affix-wrapper) {
        line-height: 38px;

        .ant-input-number-input-wrap {
            border-radius: 7px;
        }
    }

    // Use :focus-within because antd doesn't add a "ant-input-number-affix-wrapper-focused" class
    &.ant-input-number-focused,
    &.ant-input-number-affix-wrapper:focus-within {
        border-style: dashed;
        border-color: ${r.color.primary.blue400};

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            box-shadow: none;
        }
    }

    &.ant-input-number-focused,
    &.ant-input-number-affix-wrapper .ant-input-number-focused,
    &.ant-input-number-focused .ant-input-number-affix-wrapper {
        .ant-input-number-input::placeholder {
            color: transparent;
        }
    }

    &.ant-input-number-disabled,
    &.ant-input-number-affix-wrapper-disabled {
        border-color: ${r.color.secondary.mediumGrey.mediumGrey200};

        .ant-input-number-input {
            color: ${r.color.secondary.mediumGrey.mediumGrey400};

            &::placeholder {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }
        }

        &.ant-input-number-affix-wrapper .ant-input-number-prefix {
            color: ${r.color.secondary.mediumGrey.mediumGrey400};
        }
    }

    &.ant-input-number-affix-wrapper {
        .ant-input-number-input-wrap {
            .ant-input-number-input {
                padding: 0px 10px 0px 0px;
            }
        }

        .ant-input-number-prefix {
            margin-inline-end: 10px;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        .ant-input-number {
            height: 38px;
            line-height: 38px;
        }
    }

    .ant-input-number-input-wrap {
        border-radius: 0 7px 7px 0;

        .ant-input-number-input {
            padding: 0px 10px;
            border-radius: 0;
        }
    }

    &.ant-input-number-status-warning,
    &.ant-input-number-affix-wrapper-status-warning {
        background-color: ${r.color.secondary.orange.orange100};
        box-shadow: none;

        .ant-input-number-input-wrap {
            background-color: ${r.color.secondary.orange.orange100};
        }

        .ant-input-number-input {
            color: ${r.color.secondary.orange.orange400};

            &::placeholder {
                color: ${r.color.secondary.orange.orange400};
            }
        }

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${r.color.secondary.orange.orange400};

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.orange.orange400};
            }

            &.ant-input-number-affix-wrapper .ant-input-number-prefix {
                color: ${r.color.secondary.orange.orange400};
            }
        }
    }

    &.ant-input-number-status-error,
    &.ant-input-number-out-of-range,
    &.ant-input-number-affix-wrapper-status-error {
        background-color: ${r.color.secondary.red.red100};
        box-shadow: none;

        .ant-input-number-input-wrap {
            background-color: ${r.color.secondary.red.red100};
        }

        .ant-input-number-input {
            color: ${r.color.secondary.red.red400};

            &::placeholder {
                color: ${r.color.secondary.red.red400};
            }
        }

        &:not(.ant-input-borderless):not(.ant-input-disabled) {
            border-color: ${r.color.secondary.red.red400};

            &:hover,
            &:focus {
                border-color: ${r.color.secondary.red.red400};
            }

            &.ant-input-number-affix-wrapper .ant-input-number-prefix {
                color: ${r.color.secondary.red.red400};
            }
        }
    }
`), _a = ({ label: e, helper: o, ...a }) => t.createElement(de, { label: e, helper: o, disabled: a.disabled, status: a.status }, t.createElement($a, { ...a })), v = (_a.displayName = "KitInputNumber", { fontWeight: { default: r.typography.mediumfontWeight, checked: r.typography.boldFontWeight }, borderWidth: "1px", color: { default: r.color.neutral.typography.black, enabled: r.color.neutral.typography.black, disabled: r.color.secondary.mediumGrey.mediumGrey400, danger: { default: r.color.neutral.typography.black, disabled: r.color.secondary.red.red200 } }, backgroundColor: { default: r.color.neutral.typography.white, checked: r.color.neutral.typography.white, hover: r.color.neutral.typography.white, disabled: r.color.secondary.mediumGrey.mediumGrey100, danger: { default: r.color.secondary.red.red200, hover: r.color.secondary.red.red200, disabled: r.color.secondary.red.red400 } }, borderColor: { default: r.color.neutral.typography.black60, checked: r.color.primary.blue400, hover: r.color.primary.blue500, disabled: r.color.secondary.mediumGrey.mediumGrey200, danger: { default: r.color.secondary.red.red400, hover: r.color.secondary.red.red500, disabled: r.color.secondary.red.red200 } }, bulletColor: { default: r.color.primary.blue400, hover: r.color.primary.blue500, disabled: r.color.secondary.mediumGrey.mediumGrey200, danger: { default: r.color.secondary.red.red400, hover: r.color.secondary.red.red500, disabled: r.color.secondary.red.red400 } } }), en = k(ro)`
    font-weight: ${v.fontWeight.default};

    // Uncheked
    .ant-radio {
        .ant-radio-inner {
            border: ${v.borderWidth} solid ${v.borderColor.default};
            background-color: ${v.backgroundColor.default};
        }

        .ant-radio-inner:after {
            transform: scale(0.6);
            visibility: hidden;
            background-color: ${v.bulletColor.default};
        }
    }

    .ant-radio-checked::after {
        border-color: transparent;
    }

    &.ant-radio-wrapper-disabled .ant-radio .ant-radio-inner {
        border: ${v.borderWidth} solid ${v.borderColor.disabled};
        background-color: ${v.backgroundColor.disabled};

        &:after {
            background-color: ${v.bulletColor.disabled};
        }
    }

    &.ant-radio-wrapper-danger .ant-radio {
        .ant-radio-inner {
            border: ${v.borderWidth} solid ${v.borderColor.danger.default};
            background-color: ${v.backgroundColor.danger.default};

            &:after {
                background-color: ${v.bulletColor.danger.default};
            }
        }

        .ant-radio-input:hover + .ant-radio-inner {
            border-color: ${v.borderColor.danger.hover};
            background-color: ${v.backgroundColor.danger.hover};

            &:after {
                background-color: ${v.bulletColor.danger.hover};
            }
        }
    }

    &.ant-radio-wrapper-danger.ant-radio-wrapper-disabled .ant-radio .ant-radio-inner {
        border: ${v.borderWidth} solid ${v.borderColor.danger.default};
        background-color: ${v.backgroundColor.danger.default};

        &:after {
            background-color: ${v.bulletColor.danger.default};
        }
    }

    &:not(.ant-radio-wrapper-danger):not(.ant-radio-wrapper-disabled) {
        .ant-radio-checked .ant-radio-inner {
            border-color: ${v.borderColor.checked};
            background-color: ${v.backgroundColor.checked};
        }

        .ant-radio-input:hover + .ant-radio-inner {
            border-color: ${v.borderColor.hover};
            background-color: ${v.backgroundColor.hover};

            &:after {
                background-color: ${v.bulletColor.hover};
            }
        }
    }

    .ant-radio-checked .ant-radio-inner:after {
        visibility: visible;
    }

    // Focus
    &:not(.ant-radio-wrapper-disabled) .ant-radio-input:focus + .ant-radio-inner {
        border-style: dashed;
    }

    &:not(.ant-radio-wrapper-disabled):not(.ant-radio-wrapper-danger) .ant-radio-input:focus + .ant-radio-inner {
        border-color: ${v.borderColor.checked};
    }

    &:not(.ant-radio-wrapper-disabled) .ant-radio-input:focus-within + .ant-radio-inner::after {
        transform: scale(0.52);
    }

    // active

    &:not(.ant-radio-wrapper-danger) {
        .ant-radio-disabled + span {
            color: ${v.color.disabled};
        }
    }

    .ant-radio-disabled {
        color: ${v.color.disabled};
    }

    &.ant-radio-wrapper-danger {
        .ant-radio {
            &.ant-radio-disabled + span {
                color: ${v.color.danger.disabled};
            }
        }
    }

    .ant-radio-checked + span {
        font-weight: ${v.fontWeight.default};
    }
`, De = t.forwardRef((e, o) => {
  const { className: a, danger: i, ...l } = e;
  return t.createElement(en, { ref: o, className: i ? (a || "") + " ant-radio-wrapper-danger" : a, ...l });
}), rn = t.forwardRef((e, o) => {
  var { getPrefixCls: O, direction: a } = t.useContext(Ir), i = t.useContext(Wo);
  const [l, c] = zo(e.defaultValue, { value: e.value }), { prefixCls: s, className: p, rootClassName: h, options: m, buttonStyle: x = "outline", disabled: K, children: S, size: A, style: T, id: C, onMouseEnter: U, onMouseLeave: w, onFocus: H, onBlur: ce } = e, ee = O("radio", s);
  var O = ee + "-group", [g, _] = Qo(ee);
  let D = S;
  return m && 0 < m.length && (D = m.map((R) => typeof R == "string" || typeof R == "number" ? t.createElement(De, { key: R.toString(), prefixCls: ee, disabled: K, value: R, checked: l === R }, R) : t.createElement(De, { key: "radio-group-value-options-" + R.value, prefixCls: ee, disabled: R.disabled || K, value: R.value, checked: l === R.value, style: R.style }, R.label))), i = A || i, i = Ge(O, O + "-" + x, { [O + "-" + i]: i, [O + "-rtl"]: a === "rtl" }, p, h, _), g(t.createElement("div", { ...Vo(e, { aria: !0, data: !0 }), className: i, style: T, onMouseEnter: U, onMouseLeave: w, onFocus: H, onBlur: ce, id: C, ref: o }, t.createElement(Mo, { value: { onChange: (R) => {
    var se = l, Te = R.target.value, he = ("value" in e || c(Te), e).onChange;
    he && Te !== se && he(R);
  }, value: l, disabled: e.disabled, name: e.name } }, D)));
}), on = t.memo(rn), mr = De, tn = (mr.displayName = "KitRadio", mr.Group = on, k(oo)`
    ${({ $color: e, $theme: o }) => {
  switch (e) {
    case "default":
      return;
    case "red":
      return F`
                    color: ${o.colors.star.active.red};
                `;
    case "green":
      return F`
                    color: ${o.colors.star.active.green};
                `;
    case "blue":
      return F`
                    color: ${o.colors.star.active.blue};
                `;
  }
}}

    &.ant-rate .ant-rate-star {
        &:not(.ant-rate-star-half):not(.ant-rate-star-full) {
            .ant-rate-star-first {
                color: ${({ $theme: e }) => e.colors.star.default};
            }
        }

        &:not(.ant-rate-star-full) {
            .ant-rate-star-second {
                color: ${({ $theme: e }) => e.colors.star.default};
            }
        }
    }

    &.ant-rate-disabled {
        cursor: not-allowed;

        .ant-rate-star {
            cursor: not-allowed;

            &:not(.ant-rate-star-half):not(.ant-rate-star-full) {
                div[role='radio']::after,
                div[role='radio']::before {
                    content: '';
                    position: absolute;
                    width: 2px;
                    height: 95%;
                    background-color: ${({ $theme: e }) => e.colors.star.disabled};
                    transform-origin: 0 0;
                    transform: rotate(320deg);
                    left: 0;
                }

                div[role='radio']::after {
                    top: 2px;
                    padding-bottom: 4px;
                    border-radius: 2px;
                }

                div[role='radio']::before {
                    background-color: ${({ $disabledStarTransparency: e }) => e};
                    top: 5px;
                }
            }
        }
    }
`), an = ({ color: e = "default", disabledStarTransparency: o, ...a }) => {
  var i = Z().theme;
  return t.createElement(tn, { $theme: i.components.Rate, $color: e, $disabledStarTransparency: o ?? i.general.colors.neutral.typography.white, ...a });
}, nn = (an.displayName = "KitRate", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "g", attrs: { "data-name": "calendar off" }, children: [{ tag: "path", attrs: { d: "M292.57 553.14a41 41 0 0140.8-41.14h356.92a41.15 41.15 0 010 82.29H333.71a41 41 0 01-41.14-40.8zm233.14 123.43a41.15 41.15 0 110 82.29h-192a41.15 41.15 0 010-82.29zM388.57 182.86h246.86v-68.57a41.14 41.14 0 1182.28 0v68.57h68.58A109.71 109.71 0 01896 292.57v548.57a109.71 109.71 0 01-109.71 109.72H237.71A109.71 109.71 0 01128 841.14V292.57a109.71 109.71 0 01109.71-109.71h68.58v-68.57a41.14 41.14 0 1182.28 0zM210.29 841.14a27.43 27.43 0 0027.42 27.43h548.58a27.43 27.43 0 0027.42-27.43V402.29H210.29z", "data-name": "Vector 12" } }] }] }, name: "calendar", theme: "outlined" }), ln = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: nn })), dn = (ln.displayName = "KitCalendarOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M401.4 760.8a27.6 27.6 0 11-55.2 0V401.3a27.6 27.6 0 1155.2 0v359.5zm138.2 0a27.6 27.6 0 11-55.2 0V401.3a27.6 27.6 0 1155.2 0v359.5zm138.3 0a27.6 27.6 0 11-55.2 0V401.3a27.6 27.6 0 1155.2 0v359.5zm-4.3-648.2l63.4 95.1h120.7a41.57 41.57 0 010 82.9h-13.8v525.5c0 76.3-61.9 138.2-138.2 138.2H318.5c-76.3 0-138.2-61.9-138.2-138.2V290.7h-13.8a41.57 41.57 0 010-82.9H287l63.4-95.1a97.38 97.38 0 0180.5-42.9H593c32.3.1 62.4 16 80.6 42.7v.1zm-286.9 95.1h250.6l-32.8-49.1c-1.3-1.7-3.1-3.1-5.1-4.1-2-1-4.1-1.5-6.3-1.4H431c-4.5 0-10.4 2.2-11.4 5.5l-32.9 49.1zM263.2 816.1c0 30.3 24.7 54.9 55 54.9H705.6c30.3.2 55.1-24.2 55.3-54.5V290.7H263.2v525.4z" } }] }, name: "delete", theme: "outlined" }), cn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: dn })), sn = (cn.displayName = "KitDeleteOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { "clip-rule": "evenodd", d: "M558 111.6a42.1 42.1 0 00-84.2 0v434.8L342.9 415.7a42.16 42.16 0 00-59.6-1 42.16 42.16 0 00-1 59.6c.3.3.6.7 1 1l198.9 198.9a42.07 42.07 0 0059.6 0l202.7-202.7a42.07 42.07 0 000-59.6 42.07 42.07 0 00-59.6 0l-127 127V111.6zm214.3 756.1V737.6h86.8v130.1c-.1 47.9-38.9 86.6-86.7 86.8H251.7c-47.9-.1-86.6-38.9-86.8-86.8V737.6h86.8v130.1h520.6z", "fill-rule": "evenodd" } }] }, name: "download", theme: "outlined" }), pn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: sn })), mn = (pn.displayName = "KitDownloadOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M511.9 883.2c-10.2 0-20-4-27.1-11.2L75.2 462.4a38.3 38.3 0 01.9-54.3 38.4 38.4 0 0153.4 0l382.4 382.5 382.4-382.5a38.48 38.48 0 0154.3-.9 38.48 38.48 0 01.9 54.3l-.9.9L539.1 872c-7.2 7.2-17 11.2-27.2 11.2z" } }] }, name: "dropdown", theme: "outlined" }), Wr = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: mn })), un = (Wr.displayName = "KitDropdownOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M512 397.3c10.2 0 19.9 4 27.1 11.2l408.7 408.7c15.2 14.7 15.6 39 .9 54.2a38.36 38.36 0 01-54.2.9l-.9-.9L512 489.8 130.4 871.4a38.36 38.36 0 01-54.2.9 38.36 38.36 0 01-.9-54.2l.9-.9 408.7-408.7a38.2 38.2 0 0127.1-11.2z" } }] }, name: "dropup", theme: "outlined" }), gn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: un })), hn = (gn.displayName = "KitDropupOutlined", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "g", attrs: { "data-name": "edit=off" }, children: [{ tag: "path", attrs: { d: "M692 160a108.67 108.67 0 01153.92 0l18.24 18.19a109 109 0 010 154L566 630.11A112.49 112.49 0 01518.88 658L367.7 701a34.15 34.15 0 01-35.7-10.51 33.57 33.57 0 01-9.21-34.42L366 505.13A111.19 111.19 0 01393.66 458zm101.22 51a35.18 35.18 0 00-50.06 0l-44 44.1 69.64 69.64 44-45.25a35.06 35.06 0 000-50.31zM435.9 524.86l-25.39 88.59 88.41-25.52a32.21 32.21 0 0015.69-9.28l203-202.77L648 306.19 445.11 509.36a35.55 35.55 0 00-9.21 15.5zm-5.62-306.06a36.19 36.19 0 110 72.37H261.05a60.59 60.59 0 00-60.47 60.76v411a60.24 60.24 0 0060.47 60.38h411.13a60.18 60.18 0 0060.47-60.38v-169.1a36.29 36.29 0 1172.57 0v169.08A133.07 133.07 0 01672.18 896H261.05A133 133 0 01128 762.91v-411a133 133 0 01133-133.13z", "data-name": "Vector 7" } }] }] }, name: "edit", theme: "outlined" }), yn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: hn })), bn = (yn.displayName = "KitEditOutlined", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "g", attrs: { "data-name": "hide=off" }, children: [{ tag: "path", attrs: { d: "M309.51 316.4a328.87 328.87 0 01203-72.81 337.25 337.25 0 01231 96.78A463.27 463.27 0 01855 497.65a37.76 37.76 0 010 29.5 474.43 474.43 0 01-95.83 142.22L885.41 768a28.9 28.9 0 01-35.64 45.48L139.56 256.79a28.89 28.89 0 1135.63-45.47zm46.68 36.88l55.28 43.19a153.79 153.79 0 01237.74 186.18l64.52 50.66a465.36 465.36 0 0085.09-120.93 435.67 435.67 0 00-94.33-129.95 281.94 281.94 0 00-192-82.31 266.56 266.56 0 00-156.25 53.12zM602.35 546.2a102.59 102.59 0 006.14-34.56 96.7 96.7 0 00-96.14-96.13c-.9 0-1.55 1.21-3.37 1.21a48.34 48.34 0 013.37 18.14 76.35 76.35 0 01-8 35zm11.06 156.33l50.4 39.65a313.92 313.92 0 01-151.46 38.87 337.34 337.34 0 01-231-96.78A484.71 484.71 0 01169.84 527a37.76 37.76 0 010-29.5 456.71 456.71 0 0158.38-98.6l45.18 35.64a389.74 389.74 0 00-47.51 76.78 420.59 420.59 0 0094.32 130.86 281.19 281.19 0 00192 81.41 251.85 251.85 0 00101.41-21zm-254.8-191.06c0-2.46 0-5.83.3-9.19l67.28 52.82a91.24 91.24 0 0064.53 49.74L558 658.92a186.28 186.28 0 01-47 7 154.63 154.63 0 01-153.6-154.49z", "data-name": "Vector 6" } }] }] }, name: "hide", theme: "outlined" }), fn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: bn })), kn = (fn.displayName = "KitHideOutlined", { icon: { tag: "svg", attrs: { id: "Calque_1", version: "1.1", viewBox: "64 64 896 896", x: "0", "xml:space": "preserve", y: "0", focusable: "false" }, children: [{ tag: "style", attrs: {} }, { tag: "g", attrs: { id: "info_off" }, children: [{ tag: "path", attrs: { d: "M470.4 304h83.2v83.2h-83.2V304zM512 720c23 0 41.6-18.6 41.6-41.6V512c0-23-18.6-41.6-41.6-41.6S470.4 489 470.4 512v166.4c0 23 18.6 41.6 41.6 41.6z", id: "TracÃ_7602" } }, { tag: "path", attrs: { d: "M512 96C282.2 96 96 282.3 96 512c0 229.8 186.3 416 416 416s416-186.3 416-416c0-110.3-43.8-216.1-121.8-294.2A416.32 416.32 0 00512 96zm0 748.8c-184.1.3-333.5-148.7-333.7-332.8C178 327.9 327 178.5 511.1 178.3c184-.3 333.4 148.7 333.7 332.8A333.2 333.2 0 01512 844.8z", id: "TracÃ_7601" } }] }] }, name: "info", theme: "outlined" }), Vr = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: kn })), xn = (Vr.displayName = "KitInfoOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M888.8 551.2H135.2a39.2 39.2 0 110-78.4h753.6a39.2 39.2 0 110 78.4z" } }] }, name: "less", theme: "outlined" }), An = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: xn })), vn = (An.displayName = "KitLessOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M860 884.5H164c-19.9 0-36-16.1-36-36s16.1-36 36-36h696c19.9 0 36 16.1 36 36s-16.1 36-36 36zm0-336.6H164c-19.9 0-36-16.1-36-36s16.1-36 36-36h696c19.9 0 36 16.1 36 36s-16.1 36-36 36zm0-336.4H164c-19.9 0-36-16.1-36-36s16.1-36 36-36h696c19.9 0 36 16.1 36 36s-16.1 36-36 36z" } }] }, name: "list", theme: "outlined" }), Cn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: vn })), wn = (Cn.displayName = "KitListOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M292.6 402.3V292.6c0-121.2 98.2-219.4 219.4-219.4s219.4 98.2 219.4 219.4v109.7h54.9c60.6 0 109.7 49.1 109.7 109.7v329.1c0 60.6-49.1 109.7-109.7 109.7H237.7c-60.6 0-109.7-49.1-109.7-109.7V512c0-60.6 49.1-109.7 109.7-109.7h54.9zm82 0h274.7V292.6c0-75.9-61.5-137.4-137.4-137.4s-137.4 61.5-137.4 137.4v109.7zM210.5 841.1c-.1 15 12 27.1 26.9 27.2h548.9c15-.1 27.1-12.2 27.2-27.2V512c-.1-15-12.2-27.1-27.2-27.2H237.7c-15-.1-27.1 12-27.2 26.9V841.1z" } }] }, name: "lock", theme: "outlined" }), Sn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: wn })), En = (Sn.displayName = "KitLockOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M416.1 812.1c0 19.9-16.1 36-35.9 36.1H272c-79.5 0-144-64.5-144-144V319.9c0-79.6 64.5-144.1 144.1-144.1H380c19.9 0 36 16.1 36 36s-16.1 36-36 36H272c-39.7.2-71.8 32.3-72 72V704c0 39.8 32.2 72 72 72h108c19.9-.1 36 16 36.1 35.9v.2zm470.6-324.8l-191-204.1a36 36 0 00-50.8-2.3 36 36 0 00-2.3 50.8c.3.3.5.6.8.8l133.4 143.3H402.6a35.98 35.98 0 00-33.9 38.1 35.98 35.98 0 0033.9 33.9h373.1L640.4 691.2a36.48 36.48 0 001.5 50.9c7.1 5.8 15.8 9.2 25 9.6 9.9.1 19.4-4.1 26.1-11.3l190.6-204.1a33.35 33.35 0 004.9-46.9c-.5-.7-1.1-1.4-1.8-2.1z" } }] }, name: "logout", theme: "outlined" }), Fn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: En })), Kn = (Fn.displayName = "KitLogoutOutlined", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M868.94 648.83c-4.73 2.93-18.22 4.73-26.77 4.73-27.9 0-62.1-12.83-110.47-33.53 18.67-1.35 35.55-2 50.85-2 27.89 0 36 0 63.44 7 27.22 6.75 27.45 20.93 23 23.85zM385 653.11c10.8-18.9 21.82-38.92 33.07-60.3 27.45-52 45-92.91 57.82-126.44 25.87 47 58.05 86.85 95.62 118.79 4.72 4.05 9.67 7.88 15.07 11.93-76.71 15.3-143.08 33.74-201.58 56.02zm89.54-492.48c15.3 0 24.07 38.38 24.75 74.6.67 36-7.65 61.2-18.22 80.09-8.78-27.89-12.83-71.54-12.83-100.11.01 0-.66-54.58 6.31-54.58zM174.43 851.76c8.77-23.62 43-70.42 93.59-112 3.15-2.47 11-9.9 18.22-16.65-52.87 84.6-88.41 118.12-111.81 128.69zm709.13-252.65c-15.3-15.07-49.49-23-101.24-23.62-35.09-.45-77.17 2.7-121.71 8.77-19.8-11.47-40.27-23.84-56.47-38.92-43.2-40.49-79.19-96.51-101.69-158.16 1.35-5.85 2.7-10.8 3.82-16 0 0 24.3-138.36 17.78-185.16-.9-6.52-1.35-8.32-3.15-13.27l-2-5.63c-6.52-15.21-19.57-31.4-40-30.53l-11.93-.38h-.22c-22.73 0-41.4 11.63-46.12 28.89-14.85 54.67.45 136.11 28.12 241.63l-7.2 17.32c-19.8 48.14-44.55 96.74-66.37 139.49l-2.92 5.62c-22.95 45-43.87 83.24-62.77 115.64l-19.58 10.35c-1.35.9-34.87 18.45-42.74 23.17-66.6 39.82-110.87 85-118.21 121-2.34 11.25-.58 25.87 11.28 32.85l18.89 9.44a57.8 57.8 0 0025.72 6.08c47.47 0 102.59-59 178.41-191.46 87.74-28.57 187.63-52.42 275.15-65.47 66.59 37.58 148.48 63.67 200.23 63.67a89.83 89.83 0 0023.62-2.7c9.9-2.47 18.22-8.1 23.4-16 9.9-15.07 12.15-35.77 9.22-57.15-.67-6.29-5.85-14.17-11.25-19.57z" } }] }, name: "pdf", theme: "outlined" }), In = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: Kn })), Gn = (In.displayName = "KitPdfOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M512 960c-23.2 0-41.9-18.8-41.9-41.9V553.9H105.9C82.8 553.9 64 535.2 64 512s18.8-41.9 41.9-41.9H470V105.9c0-23.2 18.8-41.9 41.9-41.9 23.2 0 41.9 18.8 41.9 41.9V470h364.1c23.2 0 41.9 18.8 41.9 41.9 0 23.2-18.8 41.9-41.9 41.9h-364v364.1c0 23.3-18.7 42.1-41.9 42.1z" } }] }, name: "plus", theme: "outlined" }), Tn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: Gn })), Rn = (Tn.displayName = "KitPlusOutlined", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "g", attrs: { "data-name": "railroad=off" }, children: [{ tag: "path", attrs: { d: "M292.57 621.93a54.87 54.87 0 1116.07 38.79 54.89 54.89 0 01-16.07-38.79zm438.86 0a54.87 54.87 0 11-16.07-38.79 54.86 54.86 0 0116.07 38.79zm0-548.57A164.57 164.57 0 01896 237.93v438.86a164.56 164.56 0 01-114.16 156.72l45.75 47.29a40.12 40.12 0 11-56.77 56.72l-97.49-96.16H350.72l-97.59 96.16a38.82 38.82 0 01-58.2 0 39.37 39.37 0 010-56.72l47.34-47.29A164.56 164.56 0 01128 676.79V237.93A164.57 164.57 0 01292.57 73.36zm-260.52 82.29H292.57a82.25 82.25 0 00-82.28 82.28v192h260.57zm82.29 0v274.28h260.57v-192a82.28 82.28 0 00-82.29-82.28zM292.63 759.07h438.85a82.28 82.28 0 0082.29-82.28V512.22H210.34v164.57a82.28 82.28 0 0082.29 82.28z", "data-name": "Vector 2" } }] }] }, name: "railroad", theme: "outlined" }), Bn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: Rn })), Un = (Bn.displayName = "KitRailroadOutlined", { icon: { tag: "svg", attrs: { "data-name": "Calque 1", viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "g", attrs: { "data-name": "show=off" }, children: [{ tag: "path", attrs: { d: "M341.37 511.93a170.67 170.67 0 1113 65.32 170.66 170.66 0 01-13-65.32zM512 618.63a106.68 106.68 0 000-213.36h-3.84a53.35 53.35 0 013.84 21.36A85.39 85.39 0 01426.66 512a52.83 52.83 0 01-21.3-3.84v2.64A106.86 106.86 0 00512 618.63zM255.22 320.75a360.56 360.56 0 01513.58 0 516.29 516.29 0 01123.89 174.86 42.23 42.23 0 010 32.78A540.09 540.09 0 01768.8 703.25a360.54 360.54 0 01-513.58 0 542.24 542.24 0 01-123.94-174.86 42.59 42.59 0 010-32.78 518.16 518.16 0 01123.94-174.86zM512 277.31a312 312 0 00-213.22 90.28A460.49 460.49 0 00193.87 512a474 474 0 00104.92 144.36 296.88 296.88 0 00426.43 0A495.51 495.51 0 00830.15 512a480.38 480.38 0 00-104.93-144.41A311.93 311.93 0 00512 277.31z", "data-name": "Vector 20" } }] }] }, name: "show", theme: "outlined" }), Yn = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: Un })), zn = (Yn.displayName = "KitShowOutlined", { icon: { tag: "svg", attrs: { viewBox: "64 64 896 896", focusable: "false" }, children: [{ tag: "path", attrs: { d: "M950.2 793.7L577 156.9a73.9 73.9 0 00-64.7-36.7c-27 0-52 13.9-66.3 36.7L73 793.7a74.66 74.66 0 0030.2 101.2c11 5.9 23.3 9 35.8 9h746.4c40.6.6 74-31.7 74.7-72.3.1-13.3-3.3-26.4-9.9-37.9zm-793.6 25.9l354.3-607.4 357.6 607.4H156.6zM512.5 654c14.8-3 29.2 6.6 32.1 21.3 3 14.8-6.6 29.2-21.3 32.1a27.2 27.2 0 01-32.1-21.3 27.41 27.41 0 0120.9-32.1h.4zm-42-267.9V554a42 42 0 1084 0V386a42 42 0 10-84 0v.1z" } }] }, name: "warning", theme: "outlined" }), Mr = u.forwardRef((e, o) => u.createElement(Q, { ...e, ref: o, icon: zn })), Wn = (Mr.displayName = "KitWarningOutlined", k(to)`
    ${rt}
`), Vn = (e) => t.createElement("div", { className: "kit-select-option" }, e.icon && t.createElement(P, { className: "kit-select-option-icon", icon: e.icon, on: !0 }), !e.icon && t.createElement(_o, { className: "kit-select-option-badge" }, e.color && t.createElement("div", { className: "kit-select-option-color", style: { backgroundColor: e.color } })), t.createElement(tr, { className: "kit-select-option-label" }, e.label)), Qr = (e, o) => e.map(({ className: a, disabled: i, value: l, options: c, ...s }) => c ? { label: s.label, options: Qr(c, o) } : { label: o ? t.createElement(tr, null, s.label) : Vn(s), className: s.highlight ? a + " kit-select-highlight-option" : a, disabled: i, value: l }), Mn = (e) => t.createElement("div", { className: "kit-select-dropdown-content" }, e), Qn = (o) => {
  var { label: o, onClose: a } = o;
  return t.createElement(we, { color: "blueInvert", onMouseDown: (i) => {
    i.preventDefault(), i.stopPropagation();
  }, onClose: a }, typeof o == "object" && o, typeof o == "string" && t.createElement("div", { className: "kit-select-option" }, t.createElement(tr, { className: "kit-select-option-label" }, o)));
}, Zn = (e) => t.createElement(we, { color: "blueInvert" }, "+", e.length, " ..."), Jn = ({ options: e, labelOnly: o, label: a, helper: i, onClick: l, onClear: c, onBlur: s, oneLineTags: p = !1, allowClear: h = !0, ...m }) => {
  let [x, K] = ue([]);
  const [S, A] = ue(!1), T = Lr(null);
  return Hr(() => {
    K(e ? Qr(e, o) : []);
  }, [e, o]), t.createElement(de, { label: a, helper: i, disabled: m.disabled, status: m.status }, t.createElement(Wn, { ...m, className: (m.className || "") + " ant-select-" + (m.placement && 0 <= m.placement.indexOf("top") ? "top" : "bottom"), popupClassName: Ge({ "kit-select-dropdown-top": m.placement && 0 <= m.placement.indexOf("top"), "kit-select-dropdown-bottom": !m.placement || m.placement.indexOf("top") < 0 }), options: x, menuItemSelectedIcon: t.createElement(P, { icon: t.createElement(Sr, null), on: !0 }), suffixIcon: m.loading ? void 0 : t.createElement(Wr, null), allowClear: h ? { clearIcon: t.createElement(ge, null) } : void 0, dropdownRender: Mn, tagRender: m.mode ? Qn : void 0, maxTagCount: p ? "responsive" : void 0, maxTagPlaceholder: p ? Zn : void 0, ref: T, open: m.open !== void 0 ? m.open : S, onClick: (C) => (C = C, T.current && T.current.focus(), !m.mode && S || m.mode && S && C.target.closest(".ant-select") ? A(!1) : A(!0), void (l && l(C))), onClear: () => {
    A(!1), c && c();
  }, onBlur: (C) => (C = C, A(!1), void (s && s(C))) }));
}, Pn = (Jn.displayName = "KitSelect", k(ao)`
    padding: 4px 8px;
    border: none;
    height: 24px;

    > div {
        padding-left: 0;
    }

    &,
    a {
        font-size: ${({ $theme: e }) => e.typography.fontSize}px;
        font-family: ${({ $theme: e }) => e.typography.fontFamily};
        font-weight: ${({ $theme: e }) => e.typography.fontWeight};
        line-height: 18px;
        box-sizing: border-box;
        display: flex;
        width: fit-content;
    }

    .ant-tag-close-icon {
        margin-left: 8px;
    }

    > .anticon + span {
        margin-left: 8px;
    }

    ${({ $color: e, $theme: o }) => {
  switch (e) {
    case "default":
      return;
    case "red":
      return F`
                    background: ${o.colors.red.background.default};
                    color: ${o.colors.red.typography.default};

                    .anticon-close {
                        color: ${o.colors.red.typography.closeIcon};

                        &:hover {
                            color: ${o.colors.red.typography.hover};
                        }
                    }
                `;
    case "green":
      return F`
                    background: ${o.colors.green.background.default};
                    color: ${o.colors.green.typography.default};

                    .anticon-close {
                        color: ${o.colors.green.typography.closeIcon};

                        &:hover {
                            color: ${o.colors.green.typography.hover};
                        }
                    }
                `;
    case "blue":
      return F`
                    background: ${o.colors.blue.background.default};
                    color: ${o.colors.blue.typography.default};

                    .anticon-close {
                        color: ${o.colors.blue.typography.closeIcon};

                        &:hover {
                            color: ${o.colors.blue.typography.hover};
                        }
                    }
                `;
    case "blueInvert":
      return F`
                    background: ${o.colors.blueInvert.background.default};
                    color: ${o.colors.blueInvert.typography.default};

                    .anticon-close {
                        color: ${o.colors.blueInvert.typography.closeIcon};

                        &:hover {
                            color: ${o.colors.blueInvert.typography.hover};
                        }
                    }
                `;
  }
}}
`), qn = ({ color: e = "default", ...o }) => {
  var a = Z().theme;
  return t.createElement(Pn, { ...o, $theme: a.components.Tag, $color: e, closable: !!o.onClose });
}, we = qn, I = (we.displayName = "KitTag", { fontWeight: { default: r.typography.mediumfontWeight, checked: r.typography.boldFontWeight }, borderWidth: "1px", width: 32, height: 20, iconFontSize: 12, pinHeight: 16, handle: { borderWidth: "1.3px", borderColor: { default: { off: r.color.secondary.mediumGrey.mediumGrey400, on: r.color.primary.blue400 }, disabled: { off: r.color.secondary.mediumGrey.mediumGrey200, on: r.color.secondary.mediumGrey.mediumGrey200 }, focus: { off: r.color.primary.blue200, on: r.color.primary.blue200 } }, backgroundColor: { default: { on: r.color.neutral.typography.white, off: r.color.secondary.mediumGrey.mediumGrey400 }, disabled: r.color.secondary.mediumGrey.mediumGrey100 } }, backgroundColor: { default: { on: r.color.primary.blue400, off: r.color.neutral.background }, disabled: { on: r.color.secondary.mediumGrey.mediumGrey100, off: r.color.secondary.mediumGrey.mediumGrey100 }, focus: { on: "", off: "" } } }), Nn = k(no)`
    & {
        background: ${I.backgroundColor.default.off};
        border: ${I.handle.borderWidth} solid ${I.handle.borderColor.default.off};
        height: ${I.height}px;
        width: 32px;
        min-width: 32px;
        border-radius: 20px;
    }

    &.ant-switch-checked {
        border: ${I.handle.borderWidth} solid ${I.handle.borderColor.default.on};
    }

    &.ant-switch:hover {
        background-color: white;
    }

    & {
        height: 22px;
    }

    .ant-switch-handle {
        font-size: ${I.iconFontSize}px;
        top: 1px;
        width: 18px;
        height: 18px;
        inset-inline-start: 1px;

        .ant-switch-loading-icon {
            top: ${(I.pinHeight - I.iconFontSize) / 2 + 1}px;
        }
    }

    &.ant-switch-checked .ant-switch-handle {
        inset-inline-start: calc(100% - 19px);
    }

    .ant-switch-handle::before {
        border: ${I.handle.borderWidth} solid ${I.handle.borderColor.default.off};
        background-color: ${I.handle.backgroundColor.default.off};
        box-shadow: none;
        border-radius: 9px;
    }

    &.ant-switch-checked .ant-switch-handle::before {
        border: ${I.handle.borderWidth} solid ${I.handle.borderColor.default.on};
        background-color: ${I.handle.backgroundColor.default.on};
    }

    &.ant-switch-disabled,
    &.ant-switch-checked.ant-switch-disabled {
        background: ${I.backgroundColor.disabled.on};
        border-color: ${I.handle.borderColor.disabled.on};

        .ant-switch-handle::before {
            border-color: ${I.handle.borderColor.disabled.on};
            background-color: ${I.handle.borderColor.disabled.on};
            box-shadow: none;
        }
    }

    &.ant-switch.ant-switch-checked:not(.ant-switch-disabled) {
        background: ${I.backgroundColor.default.on};

        .ant-switch-handle::before {
            border-color: ${I.handle.borderColor.default.on};
            box-shadow: none;
        }

        &.ant-switch.ant-switch-focus {
            .ant-switch-handle::before {
                box-shadow: none;
            }
        }
    }

    // Focus
    &.ant-switch-checked:focus-visible .ant-switch-handle {
        width: 23px;
        inset-inline-start: calc(100% - 24px);
    }

    &.ant-switch:not(.ant-switch-checked):focus-visible .ant-switch-handle {
        width: 23px;
        &::before {
            background-color: ${I.handle.borderColor.default.on};
            border-color: ${I.handle.borderColor.default.on};
        }
    }
`, Xn = (e) => t.createElement(Nn, { ...e, checkedChildren: null, unCheckedChildren: null, size: "default" }), Ln = (Xn.displayName = "KitSwitch", k(Ar)`
    &.ant-picker {
        height: 40px;
        padding: 0px 12px 0px 8px;
        min-width: 165px;
        font-weight: ${r.typography.regularFontWeight};

        .ant-picker-input {
            display: grid;
            grid-template-areas: 'icon input clear';
            grid-template-columns: 28px 1fr 12px;

            input {
                grid-area: input;
                font-weight: ${r.typography.mediumfontWeight};
                font-family: ${r.typography.fontFamily};

                &::placeholder {
                    font-weight: ${r.typography.regularFontWeight};
                }
            }

            .ant-picker-suffix {
                grid-area: icon;
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }
            .ant-picker-clear {
                grid-area: clear;
                font-size: 12px;
                position: initial;
                top: initial;
                line-height: 1;
                transform: none;
                transition: none;
                opacity: 1;
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }
        }

        &.ant-picker-disabled {
            border-color: ${r.color.secondary.mediumGrey.mediumGrey200};

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.mediumGrey.mediumGrey400};

                    &::placeholder {
                        color: ${r.color.secondary.mediumGrey.mediumGrey400};
                    }
                }

                .ant-picker-suffix,
                .ant-picker-clear {
                    color: ${r.color.secondary.mediumGrey.mediumGrey400};
                }
            }
        }

        &.ant-picker-status-warning:not(.ant-picker-disabled) {
            background-color: ${r.color.secondary.orange.orange100};
            border-color: ${r.color.secondary.orange.orange400};
            box-shadow: none;

            &:hover {
                background-color: ${r.color.secondary.orange.orange100};
                border-color: ${r.color.secondary.orange.orange400};
            }

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.orange.orange400};

                    &::placeholder {
                        color: ${r.color.secondary.orange.orange400};
                    }
                }

                .ant-picker-suffix {
                    color: ${r.color.secondary.orange.orange400};
                }

                .ant-picker-clear {
                    color: ${r.color.secondary.orange.orange500};
                }
            }
        }

        &.ant-picker-status-error:not(.ant-picker-disabled) {
            background-color: ${r.color.secondary.red.red100};
            border-color: ${r.color.secondary.red.red400};
            box-shadow: none;

            &:hover {
                background-color: ${r.color.secondary.red.red100};
                border-color: ${r.color.secondary.red.red400};
            }

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.red.red400};

                    &::placeholder {
                        color: ${r.color.secondary.red.red400};
                    }
                }

                .ant-picker-suffix,
                .ant-picker-clear {
                    color: ${r.color.secondary.red.red400};
                }
            }
        }

        &.ant-picker-focused {
            border-style: dashed;
            box-shadow: none;

            &:not(.ant-picker-disabled) .ant-picker-input input::placeholder {
                color: transparent;
            }
        }
    }
`), Hn = ({ label: e, helper: o, allowClear: a = !0, ...i }) => t.createElement(de, { label: e, helper: o, disabled: i.disabled, status: i.status }, t.createElement(Ln, { ...i, allowClear: !!a && { clearIcon: t.createElement(ge, null) } })), On = k(Ar.RangePicker)`
    &.ant-picker.ant-picker-range {
        display: grid;
        grid-template-areas: 'icon input1 separator input2 clear';
        grid-template-columns: 28px 1fr 28px 1fr 12px;
        height: 40px;
        min-width: 270px;
        padding: 0px 12px 0px 8px;
        font-weight: ${r.typography.regularFontWeight};

        .ant-picker-range-separator {
            grid-area: separator;

            .ant-picker-separator {
                color: ${r.color.secondary.mediumGrey.mediumGrey500};
            }
        }

        .ant-picker-active-bar {
            display: none;
        }

        .ant-picker-input {
            &:nth-child(1) {
                grid-area: input1;
            }

            &:nth-child(2) {
                grid-area: input2;
            }
        }

        .ant-picker-input {
            input {
                font-weight: ${r.typography.mediumfontWeight};
                font-family: ${r.typography.fontFamily};

                &::placeholder {
                    font-weight: ${r.typography.regularFontWeight};
                }
            }
        }

        .ant-picker-suffix {
            grid-area: icon;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        .ant-picker-clear {
            grid-area: clear;
            font-size: 12px;
            position: initial;
            top: initial;
            line-height: 1;
            transform: none;
            transition: none;
            opacity: 1;
            color: ${r.color.secondary.mediumGrey.mediumGrey500};
        }

        .ant-picker-input {
            input[disabled] {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }
        }

        &.ant-picker-disabled {
            border-color: ${r.color.secondary.mediumGrey.mediumGrey200};

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.mediumGrey.mediumGrey400};

                    &::placeholder {
                        color: ${r.color.secondary.mediumGrey.mediumGrey400};
                    }
                }
            }

            .ant-picker-range-separator .ant-picker-separator {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }

            .ant-picker-suffix,
            .ant-picker-clear {
                color: ${r.color.secondary.mediumGrey.mediumGrey400};
            }
        }

        &.ant-picker-status-warning:not(.ant-picker-disabled) {
            background-color: ${r.color.secondary.orange.orange100};
            border-color: ${r.color.secondary.orange.orange400};
            box-shadow: none;

            &:hover {
                background-color: ${r.color.secondary.orange.orange100};
                border-color: ${r.color.secondary.orange.orange400};
            }

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.orange.orange400};

                    &::placeholder {
                        color: ${r.color.secondary.orange.orange400};
                    }
                }
            }

            .ant-picker-range-separator .ant-picker-separator {
                color: ${r.color.secondary.orange.orange300};
            }

            .ant-picker-suffix {
                color: ${r.color.secondary.orange.orange400};
            }

            .ant-picker-clear {
                color: ${r.color.secondary.orange.orange500};
            }
        }

        &.ant-picker-status-error:not(.ant-picker-disabled) {
            background-color: ${r.color.secondary.red.red100};
            border-color: ${r.color.secondary.red.red400};
            box-shadow: none;

            &:hover {
                background-color: ${r.color.secondary.red.red100};
                border-color: ${r.color.secondary.red.red400};
            }

            .ant-picker-input {
                input {
                    color: ${r.color.secondary.red.red400};

                    &::placeholder {
                        color: ${r.color.secondary.red.red400};
                    }
                }
            }

            .ant-picker-range-separator .ant-picker-separator {
                color: ${r.color.secondary.red.red300};
            }

            .ant-picker-suffix {
                color: ${r.color.secondary.red.red400};
            }

            .ant-picker-clear {
                color: ${r.color.secondary.red.red400};
            }
        }

        &.ant-picker-focused {
            border-style: dashed;
            box-shadow: none;

            &:not(.ant-picker-disabled) .ant-picker-input:focus-within input::placeholder {
                color: transparent;
            }
        }
    }
`, Dn = ({ label: e, helper: o, allowClear: a = !0, ...i }) => t.createElement(de, { label: e, helper: o, disabled: i.disabled !== void 0 && (typeof i.disabled == "boolean" ? i.disabled : i.disabled[0] && i.disabled[1]), status: i.status }, " ", t.createElement(On, { ...i, allowClear: !!a && { clearIcon: t.createElement(ge, null) } })), ur = Hn, jn = (ur.displayName = "KitDatePicker", ur.RangePicker = Dn, ({ allowClear: e, label: o, helper: a, autoFocus: i, defaultValue: l, disabled: c, placeholder: s, status: p, value: h, onBlur: m, onChange: x, popupClassName: K, ...S }) => t.createElement(de, { label: o, helper: a, disabled: c, status: p }, t.createElement(io, { ...S, disabled: c, popupClassName: "ant-select-dropdown kit-select-dropdown-bottom " + (K || "") }, t.createElement(Ke, { prefix: t.createElement(Er, null), allowClear: e, autoFocus: i, defaultValue: l, placeholder: s, status: p, value: h, onBlur: m })))), re = (jn.displayName = "KitAutoComplete", r).color, $n = k(er)`
    &.ant-upload-wrapper {
        &.ant-upload-picture-card-wrapper {
            .ant-upload.ant-upload-select {
                height: 104px;
                width: 104px;
                border-radius: 2px;
                border-color: ${re.primary.blue400};
                background-color: ${re.secondary.mediumGrey.mediumGrey100};
            }

            .ant-upload-list {
                .ant-upload-list-picture-card {
                    .ant-upload-list-item {
                        border-radius: 2px;
                        border: 1px solid ${re.neutral.gray.gray400};
                        &:hover::before {
                            width: 86px;
                            height: 86px;
                            background-color: rgba(6, 32, 100, 0.5);
                        }

                        &.ant-upload-list-item-uploading {
                            border: 1px dashed ${re.primary.blue400};
                        }
                    }
                    .ant-upload-list-item-container {
                        height: 104px;
                        width: 104px;
                    }
                    .ant-upload-list-item-thumbnail img {
                        height: 86px;
                        width: 86px;
                    }
                    .ant-upload-list-item-error {
                        border-color: ${re.secondary.red.red400};
                        color: ${re.secondary.red.red400};
                    }
                }
                .ant-upload-list-picture {
                    .ant-upload-list-item-container {
                        .ant-upload-list-item .ant-upload-list-item-uploading {
                            border-radius: 200px;
                        }
                    }
                }
            }
        }
        .ant-upload-list .ant-upload-list-item {
            border-radius: 2px;
            padding: ${({ $listType: e }) => e === "text" || e === void 0 ? "4px" : "auto"};
        }

        .ant-upload-list-item-progress {
            padding-right: ${({ $listType: e }) => e === "text" || e === void 0 ? "4px" : "auto"};
        }

        .ant-upload-list-item-done {
            border-color: ${re.neutral.gray.gray400};
            .ant-upload-list-item-name {
                color: ${re.primary.blue400};
            }
        }

        .ant-upload-list-item-error {
            color: ${re.secondary.red.red400};
        }
    }
`, Zr = ({ listType: e = "text", loading: o, imageUrl: a, buttonWording: i, showUploadList: l, ...c }) => (i = i ?? "Upload", t.createElement($n, { $listType: e, listType: e, showUploadList: l, ...c }, (e === void 0 || e === "text" || e === "picture") && t.createElement(te, { icon: t.createElement(Eo, null) }, i), l && e === "picture-card" && t.createElement("div", null, t.createElement(Le, null), t.createElement("div", { style: { marginTop: 8 } }, i)), !l && e === "picture-card" && (a ? t.createElement("div", { style: { width: "100%", padding: "8px" } }, t.createElement("img", { src: a, alt: "avatar", style: { width: "100%" } })) : t.createElement("div", null, o ? t.createElement(Fo, null) : t.createElement(Le, null), t.createElement("div", { style: { marginTop: 8 } }, i))))), _n = (Zr.displayName = "KitUpload", er).Dragger, { color: ze, typography: Je } = r, ei = k(_n)`
    .ant-upload {
        border-radius: 2px;
        .ant-upload-btn {
            ${ze.secondary.mediumGrey.mediumGrey100}
        }
        .ant-upload-text {
            color: #000;
            font-size: ${Je.fontSize5};
            line-height: ${Je.lineHeight5};
            font-weight: ${Je.regularFontWeight};
        }
        .ant-upload-hint {
            color: ${ze.neutral.gray.gray400} !important;
            margin-top: 8px;
            margin-bottom: 0;
        }
        .ant-upload-drag-icon {
            margin-top: 0;
            margin-bottom: 22px;
        }
    }
    .ant-upload-wrapper .ant-upload-drag {
        border: 1px solid ${ze.neutral.gray.gray400};
    }

    .ant-upload-list-item-name {
        color: ${ze.primary.blue400};
    }
`, ri = ({ title: e, description: o, ...a }) => (e = e ?? "Click or drag file to this area to upload", o = o ?? "Support for a single or bulk upload. Strictly prohibited from uploading company data or other banned files.", t.createElement(ei, { ...a }, t.createElement("p", { className: "ant-upload-drag-icon" }, t.createElement(Ko, null)), t.createElement("p", { className: "ant-upload-text" }, e), t.createElement("p", { className: "ant-upload-hint" }, o))), Pe = Zr, oi = (Pe.displayName = "KitUpload", Pe.KitDragger = ri, Pe.LIST_IGNORE = er.LIST_IGNORE, k.div`
    position: relative;
`), ti = k.div`
    position: absolute;
    top: -2px;
    left: 0;
    color: ${({ $theme: e }) => e.startIcon.colors.default};
`, ai = k.div`
    position: absolute;
    top: -2px;
    right: 0;
    color: ${({ $theme: e }) => e.endIcon.colors.default};
`, gr = k(lo)`
    margin-left: ${({ $isStartIcon: e }) => e ? "30px" : "0px"};
    margin-right: ${({ $isEndIcon: e }) => e ? "30px" : "0px"};
    &.ant-slider:not(.ant-slider-disabled) {
        .ant-slider-handle {
            &:hover::after {
                background-color: ${({ $theme: e }) => e.handle.colors.background.hover};
            }
            &::after {
                box-shadow: 0 0 0 2px ${({ $theme: e }) => e.handle.colors.border.default};
            }
            &:focus::after {
                background-color: ${({ $theme: e }) => e.handle.colors.background.focus};
            }
        }

        &:hover {
            .ant-slider-dot:not(.ant-slider-dot-active) {
                border-color: ${({ $theme: e }) => e.dot.colors.border.default};
            }
        }

        .ant-slider-mark .ant-slider-mark-text:not(.ant-slider-mark-text-active) {
            color: ${({ $theme: e }) => e.dot.colors.border.default};
        }

        .ant-slider-mark {
            top: 14px;
        }
    }

    &.ant-slider-disabled {
        .ant-slider-handle::after {
            box-shadow: 0 0 0 2px ${({ $theme: e }) => e.handle.colors.border.disabled};
        }
    }
`, Kl = ({ startIcon: e, endIcon: o, marks: a, ...i }) => {
  var l = Z().theme;
  return e !== void 0 || o !== void 0 ? t.createElement(oi, null, t.createElement(ti, { $theme: l.components.Slider }, e), t.createElement(gr, { $theme: l.components.Slider, $isStartIcon: !!e, $isEndIcon: !!o, marks: a, ...i }), t.createElement(ai, { $theme: l.components.Slider }, o)) : t.createElement(gr, { marks: a, $theme: l.components.Slider, $isStartIcon: !1, $isEndIcon: !1, ...i });
}, ni = k.div`
    display: grid;
    padding: 16px;
    font-family: ${({ $theme: e }) => e.card.typography.fontFamily};
    background: ${({ $theme: e }) => e.card.colors.background.default};
    border-radius: ${({ $theme: e }) => e.card.border.radius}px;
    box-shadow: 0px 3px 8px rgba(0, 0, 0, 0.1);
    min-width: 248px;
    width: 248px;
    border: 1px solid ${({ $theme: e }) => e.card.colors.border.default};

    &:not(.kit-card-disabled):hover {
        border: 1px solid ${({ $theme: e }) => e.card.colors.border.hover};
    }

    &:not(.kit-card-disabled) {
        .kit-card-select-button {
            border-radius: 3.5px;
            font-size: 0.6rem;
            padding: 0;
            width: 16px;
            height: 16px;
            min-width: 16px;
            color: ${({ $theme: e }) => e.select.colors.typography.default};
        }
    }

    &.kit-card-disabled {
        background: ${({ $theme: e }) => e.card.colors.background.disabled};
        pointer-events: none;

        .kit-card-data {
            .kit-card-title {
                color: ${({ $theme: e }) => e.title.colors.typography.disabled};
            }
            .kit-card-desc {
                color: ${({ $theme: e }) => e.description.colors.typography.disabled};
            }
            .kit-card-footer {
                color: ${({ $theme: e }) => e.footer.colors.typography.disabled};
            }
        }

        .kit-card-select-button {
            border-radius: 3.5px;
            font-size: 0.6rem;
            background: ${({ $theme: e }) => e.select.colors.background.disabled};
            padding: 0;
            width: 16px;
            height: 16px;
            min-width: 16px;
            color: ${({ $theme: e }) => e.select.colors.typography.disabled};
        }
    }

    &.kit-card-vertical {
        grid-template:
            'picto picto'
            'select colors'
            'content content';
        grid-template-columns: max-content 1fr;

        .kit-card-select,
        .kit-card-image,
        .kit-card-icon,
        .kit-card-colorbar {
            margin-bottom: 8px;
        }

        .kit-card-image,
        .kit-card-icon,
        .kit-card-icon .kit-icon {
            width: 100%;
            height: 80px;
        }

        .kit-card-select {
            grid-auto-columns: min-content;
            column-gap: 8px;
            margin-right: 8px;
            display: flex;
            flex-direction: row;
        }
    }

    &.kit-card-horizontal {
        grid-template: 'select picto colors content';
        grid-template-columns: min-content min-content min-content 1fr;

        .kit-card-select,
        .kit-card-image,
        .kit-card-icon,
        .kit-card-colorbar {
            margin-right: 8px;
        }

        .kit-card-select {
            grid-auto-rows: min-content;
            row-gap: 8px;
        }

        .kit-card-image,
        .kit-card-icon .kit-icon {
            width: 64px;
        }

        .kit-card-icon .kit-icon {
            height: 64px;
        }

        .kit-card-colorbar {
            height: 60px;
            flex-basis: 1fr;
        }
    }

    .kit-card-select,
    .kit-card-image,
    .kit-card-picto,
    .kit-card-colorbar {
        flex: 0 0 auto;
    }

    .kit-card-select {
        grid-area: select;
        display: grid;
    }

    .kit-card-image {
        grid-area: picto;
        height: 64px;
        border-radius: ${({ $theme: e }) => e.image.border.radius}px;
        display: flex;
        align-items: center;
        justify-content: center;
        overflow: hidden;
        border: 1px solid ${({ $theme: e }) => e.image.colors.border.default};

        .kit-card-image-image {
            display: flex;
            align-items: center;
            justify-content: center;

            img {
                width: auto;
                max-width: 100%;
                max-height: 100%;
            }
        }
    }

    .kit-card-icon {
        grid-area: picto;
        height: 64px;
        width: 64px;
        border-radius: ${({ $theme: e }) => e.icon.border.radius}px;
        display: flex;
        align-items: center;
        justify-content: center;
        overflow: hidden;
        border: 1px solid ${({ $theme: e }) => e.icon.colors.border.default};

        &.noBorder {
            border-color: transparent;
        }

        .anticon {
            font-size: 2rem;
            text-align: center;
            width: 100%;
            height: 100%;
            display: inline-flex;
            align-items: center;
            justify-content: center;
        }

        .kit-icon {
            padding: 0px;
        }
    }

    .kit-card-colorbar {
        grid-area: colors;
    }

    .kit-card-data {
        grid-area: content;
        display: flex;
        flex-direction: column;
        overflow: hidden;

        > :not(:last-child) {
            margin-bottom: 5px;
        }

        .kit-card-description-container {
            position: relative;
        }

        .kit-card-title {
            font-size: ${({ $theme: e }) => e.title.typography.fontSize}px;
            font-weight: ${({ $theme: e }) => e.title.typography.fontWeight};
            color: ${({ $theme: e }) => e.title.colors.typography.default};
        }
        .kit-card-desc {
            font-size: ${({ $theme: e }) => e.description.typography.fontSize}px;
            font-weight: ${({ $theme: e }) => e.description.typography.fontWeight};
            color: ${({ $theme: e }) => e.description.colors.typography.default};

            .ant-typography-expand {
                visibility: hidden;
            }
        }
        .kit-card-footer {
            font-size: ${({ $theme: e }) => e.footer.typography.fontSize}px;
            font-weight: ${({ $theme: e }) => e.footer.typography.fontWeight};
            color: ${({ $theme: e }) => e.footer.colors.typography.default};
        }

        .kit-card-description-collexp {
            color: ${({ $theme: e }) => e.expend.colors.typography.default};

            &:hover {
                color: ${({ $theme: e }) => e.expend.colors.typography.hover};
            }

            &.kit-card-description-collapse {
                float: right;
            }

            &.kit-card-description-expand {
                position: absolute;
                right: 0;
                bottom: 0;
            }
        }
    }
`, ii = (e) => {
  if (!e)
    return null;
  let o = !1, a = {}, i = "kit-card-icon";
  switch (e.type.displayName) {
    case "KitImage":
      a = { preview: { mask: t.createElement(rr, null) }, width: "100%", height: "100%", rootClassName: (e.props.rootClassName ?? "") + " kit-card-image-image" }, i = "kit-card-image";
      break;
    case "KitIcon":
      o = !0, a = { on: !0 };
      break;
    case "KitAvatar":
      break;
    default:
      return null;
  }
  var l = t.cloneElement(e, a);
  return t.createElement("div", { className: i + " " + (o ? "noBorder" : "") }, l);
}, li = (e, o) => e ? e.map((a, i) => t.cloneElement(a, { key: i, type: "default", disabled: o, className: `${a.props.className ?? ""} kit-card-select-button` })) : null, di = (e, o, a) => Ge(a, "kit-card-wrapper", { "kit-card-vertical": e, "kit-card-horizontal": !e, "kit-card-disabled": o }), ci = ({ vertical: e, disabled: o, colors: a, picture: i, title: l, description: c, extrainfo: s, tags: p, actions: h, onSelectChange: m, onEdit: x, ...K }) => {
  var S = Z().theme;
  const [A, T] = ue(!1), [C, U] = ue(!1);
  return t.createElement(ni, { $theme: S.components.ItemCard, className: di(e, o, K.className ?? ""), ...K }, (m || x) && t.createElement("div", { className: "kit-card-select" }, m && t.createElement(ve, { onChange: (w) => m && m(w), disabled: o }), x && t.createElement(te, { className: "kit-card-select-button", onClick: () => x && x(), disabled: o }, t.createElement(Io, null)), li(h, o)), ii(i), a && t.createElement(wa, { colors: a, vertical: !e, className: "kit-card-colorbar" }), t.createElement("div", { className: "kit-card-data" }, t.createElement(M.Text, { className: "kit-card-title", ellipsis: { rows: 1, tooltip: !0 } }, l), t.createElement("div", { className: "kit-card-description-container" }, t.createElement(M.Paragraph, { className: "kit-card-desc", ellipsis: !A && { rows: 2, expandable: !0, onEllipsis: () => U(!0) } }, c, A && t.createElement(M.Link, { className: "kit-card-description-collexp kit-card-description-collapse", onClick: (w) => {
    w.stopPropagation(), T(!1);
  } }, "Less")), C && !A && t.createElement(M.Link, { className: "kit-card-description-collexp kit-card-description-expand", onClick: (w) => {
    w.stopPropagation(), T(!0);
  } }, "More")), t.createElement(M.Text, { className: "kit-card-footer" }, s), p && t.createElement("div", { className: "kit-card-tags" }, p.map((w) => t.createElement(we, { key: w, color: "blue" }, w)))));
}, si = (ci.displayName = "KitItemCard", k(co)`
    box-shadow: 0px 3px 8px 0px rgba(0, 0, 0, 0.1);

    .ant-collapse-item .ant-collapse-header {
        align-items: center;
    }

    .ant-collapse-item:first-of-type,
    .ant-collapse-item:first-of-type .ant-collapse-header {
        border-top-left-radius: ${({ $theme: e }) => e.border.radius}px;
        border-top-right-radius: ${({ $theme: e }) => e.border.radius}px;
    }

    .ant-collapse-item:last-of-type {
        &:not(.ant-collapse-item-active),
        &:not(.ant-collapse-item-active) .ant-collapse-header {
            border-bottom-left-radius: ${({ $theme: e }) => e.border.radius}px;
            border-bottom-right-radius: ${({ $theme: e }) => e.border.radius}px;
        }

        &.ant-collapse-item-active .ant-collapse-header {
            border-bottom-left-radius: 0px;
            border-bottom-right-radius: 0px;
        }
    }

    .ant-collapse-item-active {
        outline: solid 1px ${({ $theme: e }) => e.colors.border.active};
        border-bottom: none;

        > .ant-collapse-header {
            background-color: ${({ $theme: e }) => e.colors.background.active};
        }

        > .ant-collapse-content {
            border-color: ${({ $theme: e }) => e.colors.border.active};
        }
    }
`), Jr = (e) => {
  var o = Z().theme;
  return t.createElement(si, { $theme: o.components.Collapse, ...e });
}, pi = (Jr.displayName = "KitCollapse", k.div`
    display: grid;
    grid-template: 'icon image content tag';
    grid-template-columns: min-content min-content min-content min-content;
    align-items: center;
    max-height: 90px;

    .kit-collapse-header-icon {
        padding: 5px;
        grid-area: icon;
    }

    .kit-collapse-header-image {
        padding: 0 8px;
        margin-left: 8px;
        grid-area: image;
    }

    .kit-collapse-header-content {
        margin-left: 8px;
        grid-area: content;
        width: max-content;
        display: grid;
        grid-template: 'title' 'description';

        &.kit-collapse-header-content-rows {
            grid-row-gap: 10px;
        }

        .kit-collapse-header-content-title {
            grid-area: title;
        }

        .kit-collapse-header-content-description {
            grid-area: description;
        }
    }

    .kit-collapse-header-tag {
        margin-left: 8px;
        grid-area: tag;
    }
`), Pr = ({ icon: e, imageSrc: o, title: a, description: i, tagContent: l }) => t.createElement(pi, null, e !== void 0 && t.createElement("div", { className: "kit-collapse-header-icon" }, e), o !== void 0 && t.createElement("div", { className: "kit-collapse-header-image", onClick: (c) => c.stopPropagation() }, t.createElement(je, { src: o, style: { maxHeight: "90px", width: "auto" }, preview: { mask: t.createElement(rr, null) } })), (e = "kit-collapse-header-content", e += a !== void 0 && i !== void 0 ? " kit-collapse-header-content-rows" : "", (a !== void 0 || i !== void 0) && t.createElement("div", { className: e }, a !== void 0 && t.createElement(M.Text, { className: "kit-collapse-header-content-title", size: "large", weight: "medium", ellipsis: { rows: 1, tooltip: !0 } }, a), i !== void 0 && t.createElement(M.Text, { className: "kit-collapse-header-content-description", size: "large", weight: "regular", ellipsis: { rows: 1, tooltip: !0 } }, i))), l !== void 0 && t.createElement("div", { className: "kit-collapse-header-tag" }, t.createElement(we, null, l))), qe = (Pr.displayName = "KitHeader", { fontSize: r.typography.fontSize5, fontFamily: r.typography.fontFamily, fontWeight: r.typography.regularFontWeight, color: r.color.neutral.gray.gray500, lastItemColor: r.color.primary.blue400 }), mi = k(so)`
    li:not(:last-child) a,
    li:not(:last-child) .ant-breadcrumb-link,
    li:not(:last-child) .ant-breadcrumb-link a {
        font-family: ${qe.fontFamily};
        font-weight: ${qe.fontWeight};
        color: ${qe.color};
    }
`, ui = (e) => t.createElement(mi, { ...e }), gi = (ui.displayName = "KitBreadcrumb", k(po)``), nr = (e) => t.createElement(gi, { ...e }), hr = (nr.displayName = "KitDropDown", { color: r.color.neutral.typography.black, backgroundColor: r.color.neutral.typography.white, inputColor: r.color.secondary.mediumGrey.mediumGrey500 }), hi = G(mo)`
    &.ant-divider {
        font-weight: ${r.typography.mediumfontWeight};

        ${(e) => e.$noMargin && F`
                margin: 0px;
            `}

        ${(e) => e.$color === "lightGrey" && F`
                border-color: ${r.color.secondary.mediumGrey.mediumGrey100};
            `}
    }
`, yi = ({ noMargin: e = !1, color: o = "default", ...a }) => t.createElement(hi, { ...a, $noMargin: e, $color: o }), bi = (yi.displayName = "KitDivider", (e) => t.createElement(uo, { ...e })), fi = (e) => t.createElement(go, { ...e }), Il = { Row: bi, Col: fi, displayName: "KitGrid" }, Ne = { smallGap: 8, middleGap: 16, largeGap: 24 }, ki = { small: Ne.smallGap, middle: Ne.middleGap, large: Ne.largeGap }, xi = (e) => {
  const { size: o, ...a } = e;
  return e = t.useMemo(() => typeof o == "string" ? ki[o] : o, [o]), t.createElement(vr, { size: e, ...a });
}, me = xi, Ai = (me.displayName = "KitSpace", me.Compact = vr.Compact, k.div`
    padding: 16px 32px;
    background: ${hr.backgroundColor};
    display: grid;
    grid-template-areas:
        'breadcrumb breadcrumb breadcrumb'
        'title search actions';
    grid-template-columns: min-content 1fr max-content;
    grid-template-rows: 1fr min-content;
    box-shadow: 0px 3px 8px 0px rgba(0, 0, 0, 0.1);
    min-height: 40px;

    .kit-header-title {
        grid-area: title;
        overflow: hidden;
        white-space: nowrap;
        text-overflow: ellipsis;
        align-self: center;
        margin: 0;
        margin-right: 32px;
    }

    .kit-header-search {
        grid-area: search;
        max-width: 422px;

        input::placeholder {
            color: ${hr.inputColor};
        }
    }

    .kit-header-breadcrumb {
        grid-area: breadcrumb;
        padding-top: 8px;
    }

    .kit-header-actions {
        grid-area: actions;
        flex: 1 1 auto;
        align-items: center;
        justify-content: end;
        display: flex;
        padding-left: 8px;
    }

    .kit-header-plus-button {
        width: 40px;
    }

    .kit-header-action-select {
        min-width: 75px;
    }
`), vi = (e, o) => e || o ? (e = e ? e.map((a, i) => a.type.displayName !== "KitButton" ? null : t.cloneElement(a, { type: "segmented", key: i, ...a.props })) : null, t.createElement("div", { className: "kit-header-actions" }, t.createElement(me, null, e, o && t.createElement(te, { type: "primary", className: "kit-header-plus-button", onClick: o, icon: t.createElement(Le, null) })))) : null, Ci = ({ title: e, search: o, breadcrumb: s, actions: i, onPlusClick: l, ...c }) => {
  var s = s ? t.cloneElement(s, { className: "kit-header-breadcrumb" }) : null, p = Or(() => vi(i, l), [i, l]);
  return t.createElement(Ai, { ...c }, e && t.createElement(M.Title, { level: 2, className: "kit-header-title" }, e), o && t.createElement("div", { className: "kit-header-search" }, t.createElement(Ke, { ...o, prefix: t.createElement(Er, { rev: "" }) })), s, p);
}, wi = (Ci.displayName = "KitHeader", G.div`
    padding: 16px 19px;
    background-color: ${r.color.neutral.typography.white};

    display: grid;
    grid-template-areas: 'header' 'content';

    .kit-menu-header {
        grid-area: header;
        display: grid;

        .kit-menu-header-topbar {
            display: grid;
            grid-template-areas: 'title close';
            grid-template-columns: 1fr min-content;
            align-items: center;
            margin-bottom: 16px;

            .kit-menu-header-title {
                grid-area: title;
            }

            .kit-menu-header-close {
                grid-area: close;
            }
        }

        .kit-menu-header-primary {
            display: grid;
            grid-template-areas: 'button primary-input';
            grid-template-columns: min-content 1fr;
            margin-bottom: 16px;

            .kit-menu-header-button {
                grid-area: button;
                margin-right: 16px;
            }

            .kit-menu-header-primary-input {
                grid-area: primary-input;
            }
        }

        .kit-menu-header-secondary {
            display: grid;
            grid-template-areas: 'secondary-input';
            grid-template-columns: 1fr;
            margin-bottom: 16px;

            .kit-menu-header-primary-input {
                grid-area: secondary-input;
            }
        }
    }

    .menu-content {
        grid-area: content;
    }
`), Si = ({ title: e, onCloseClick: o, segmentedButton: a, primaryInput: i, secondaryInput: l, children: c, ...s }) => {
  const p = o !== void 0;
  var h = e !== void 0 || p, m = a !== void 0 || i !== void 0, x = l !== void 0;
  return t.createElement(wi, { ...s }, t.createElement("div", { className: "kit-menu-header" }, h && t.createElement("div", { className: "kit-menu-header-topbar" }, e && t.createElement("div", { className: "kit-menu-header-title" }, t.createElement(M.Text, { size: "large", weight: "medium", ellipsis: { rows: 1, tooltip: !0 } }, e)), p && t.createElement(P, { className: "kit-menu-header-close", icon: t.createElement(or, null), hoverable: !0, onClick: () => o() })), m && t.createElement("div", { className: "kit-menu-header-primary" }, (() => {
    if (a !== void 0)
      return t.createElement("div", { className: "kit-menu-header-button" }, t.cloneElement(a, { type: "segmented" }));
  })(), (() => {
    if (i !== void 0)
      return t.createElement("div", { className: "kit-menu-header-primary-input" }, i);
  })()), x && t.createElement("div", { className: "kit-menu-header-secondary" }, (() => {
    if (l !== void 0)
      return t.createElement("div", { className: "kit-menu-header-secondary-input" }, l);
  })())), t.createElement("div", { className: "menu-content" }, c));
}, Ei = G.div`
    height: 32px;
    display: grid;
    grid-template: 'select icon title actions value rafter';
    grid-template-columns: min-content min-content minmax(0px, auto) min-content min-content min-content;
    padding: 4px 8px 4px 0px;
    background-color: ${r.color.neutral.typography.white};
    align-items: center;

    ${(e) => e.$isClickable && F`
            cursor: pointer;
        `}

    ${(e) => e.$isSelected && F`
            background-color: ${r.color.primary.blue100};
            border-right: 3px solid ${r.color.primary.blue400};
            padding-inline-end: 5px;

            .kit-item-menu-title span,
            .kit-item-menu-icon span {
                color: ${r.color.primary.blue400};
            }
        `}

    &:hover {
        background-color: ${r.color.primary.blue100};
    }

    .kit-item-menu-checkbox {
        grid-area: select;
        margin-left: 8px;
        margin-right: 8px;
    }

    .kit-item-menu-icon {
        grid-area: icon;

        span {
            color: ${r.color.secondary.mediumGrey.mediumGrey300};
        }
    }

    .kit-item-menu-title {
        grid-area: title;
        margin-left: 8px;
    }

    .kit-item-menu-actions {
        grid-area: actions;
        display: grid;
        grid-template-columns: inherit;
        margin-left: 8px;
        align-items: center;

        .kit-action-more {
            cursor: pointer;
            transform: rotate(90deg);
        }
    }

    ${(e) => {
  switch (e.$type) {
    case "cta":
      return F`
                    .kit-item-menu-title span,
                    .kit-item-menu-icon span {
                        color: ${r.color.primary.blue400};
                    }
                `;
    case "ctaDanger":
      return F`
                    .kit-item-menu-title span,
                    .kit-item-menu-icon span {
                        color: ${r.color.secondary.red.red400};
                    }

                    &:hover {
                        background-color: ${r.color.secondary.red.red100};
                    }
                `;
    case "default":
      return;
  }
}}

    .kit-item-menu-value {
        grid-area: value;
        margin-left: 8px;

        span {
            color: ${r.color.secondary.mediumGrey.mediumGrey300};
        }
    }

    .kit-item-menu-rafter {
        grid-area: rafter;
        color: ${r.color.secondary.mediumGrey.mediumGrey300};
        font-size: 12px;
        margin-left: 8px;

        &:hover {
            color: ${r.color.primary.blue400};
            cursor: pointer;
        }
    }
`, Fi = ({ type: e = "default", title: o, icon: a, value: i, actions: l, onSelectChange: c, onRafterClick: s, isSelected: p = !1, onClick: h, ...m }) => {
  var x = h !== void 0;
  const K = c !== void 0, S = s !== void 0;
  return t.createElement(Ei, { $isClickable: x, $isSelected: p, $type: e, onClick: (A) => {
    A.stopPropagation(), h && h();
  }, ...m }, K && t.createElement("div", { className: "kit-item-menu-checkbox" }, t.createElement(ve, { onClick: (A) => A.stopPropagation(), onChange: (A) => {
    c && c(A);
  } })), a && t.createElement("div", { className: "kit-item-menu-icon" }, t.createElement(P, { icon: a, on: p })), o && t.createElement("div", { className: "kit-item-menu-title" }, t.createElement(M.Text, { size: "large", weight: "medium", ellipsis: { rows: 1, tooltip: !0 } }, o)), (() => {
    if (l) {
      const A = l[0] || null, T = l.length <= 2 && l[1] ? l[1] : null;
      return (l == null ? void 0 : l.length) && t.createElement("div", { className: "kit-item-menu-actions" }, A && t.createElement(ne, { title: A.label }, t.createElement(P, { icon: A.icon, hoverable: !0, onClick: (C) => {
        C.stopPropagation(), A.onClick();
      } })), T && t.createElement(ne, { title: T.label }, t.createElement(P, { icon: T.icon, hoverable: !0, onClick: (C) => {
        C.stopPropagation(), T.onClick();
      } })), 2 < l.length && t.createElement("div", { onClick: (C) => {
        C.stopPropagation();
      } }, t.createElement(nr, { menu: { items: (() => {
        var C, U;
        if (l !== void 0 && l.length !== 0)
          return (C = [...l]).splice(0, 1), U = C.map((w, H) => ({ key: H, icon: w.icon, label: w.label, onClick: w.onClick }));
      })() } }, t.createElement(ne, { title: "More" }, t.createElement(P, { className: "kit-action-more", icon: t.createElement(Fr, null), hoverable: !0 })))));
    }
  })(), i && t.createElement("div", { className: "kit-item-menu-value" }, t.createElement(M.Text, { size: "large", weight: "regular", ellipsis: { rows: 1, tooltip: !0 } }, i)), S && t.createElement("div", { className: "kit-item-menu-rafter", onClick: (A) => {
    A.stopPropagation(), s && s();
  } }, t.createElement(Kr, { rev: "" })));
}, yr = Si, Ki = (yr.displayName = "KitMenu", yr.Item = Fi, G(ho)`
    .ant-steps-item:not(.ant-steps-item-active) {
        .ant-steps-item-container[role='button']:hover {
            .ant-steps-item-icon {
                .ant-steps-icon {
                    .ant-steps-icon-dot {
                        background: ${r.color.primary.blue300};
                    }
                }
            }

            .ant-steps-item-content {
                .ant-steps-item-title,
                .ant-steps-item-description {
                    color: ${r.color.primary.blue300};
                }
            }
        }
    }
`), Ii = (e) => t.createElement(Ki, { progressDot: !0, ...e }), V = (Ii.displayName = "KitSteps", { bordered: { borderColor: r.color.neutral.typography.black60, hoverBorderColor: r.color.primary.blue400, backgroundColor: r.color.neutral.typography.white }, miniActiveBackground: "transparent", disabledActiveTextColor: r.color.neutral.typography.white, hoverColor: r.color.primary.blue400, fontSize: r.typography.fontSize5 }), Gi = k(yo)`
    &.kit-pagination-bordered:not(.ant-pagination-mini) {
        .ant-pagination-item:not(.ant-pagination-item-active) {
            border-color: ${V.bordered.borderColor};
        }

        &:not(.ant-pagination-disabled) .ant-pagination-item:hover {
            border-color: ${V.bordered.hoverBorderColor};
            background-color: ${V.bordered.backgroundColor};
        }

        .ant-pagination-next,
        .ant-pagination-prev {
            background-color: ${V.bordered.backgroundColor};
            border: 1px solid ${V.bordered.borderColor};
        }
    }

    &.ant-pagination-mini:not(.ant-pagination-disabled) {
        .ant-pagination-item,
        .ant-pagination-item-active,
        .ant-pagination-next,
        .ant-pagination-pre {
            background: ${V.miniActiveBackground};
        }
    }

    .ant-pagination-next,
    .ant-pagination-prev {
        .ant-pagination-item-link {
            font-size: 11px;
        }

        a {
            padding: 0 8px;
        }
    }

    &:not(.ant-pagination-disabled) {
        .ant-pagination-next,
        .ant-pagination-prev {
            background-color: ${V.bordered.backgroundColor};
            border: 1px solid transparent;

            a {
                color: inherit;
            }

            &:hover {
                border-color: ${V.bordered.hoverBorderColor};
                color: ${V.hoverColor};

                .ant-pagination-item-link {
                    background-color: ${V.bordered.backgroundColor};
                }

                a {
                    color: ${V.hoverColor};
                }
            }
        }
    }

    &.ant-pagination-simple {
        .ant-pagination-next,
        .ant-pagination-prev {
            .ant-pagination-item-link {
                height: 22px;
            }
        }
    }

    &.ant-pagination-disabled {
        .ant-pagination-item-active a {
            color: ${V.disabledActiveTextColor};
        }
    }

    & .kit-pagination-prev,
    & .kit-pagination-next {
        font-size: ${0.75 * V.fontSize}px;
    }

    & .ant-pagination-options-quick-jumper {
        height: 24px;
        line-height: 24px;

        input {
            height: 24px;
            border-color: ${V.bordered.borderColor};
        }
    }

    & .ant-pagination-options-size-changer {
        height: 24px;

        .ant-select-selector {
            height: 24px;
            background: ${V.bordered.backgroundColor};
            border-color: ${V.bordered.borderColor};

            input,
            .ant-select-selection-item {
                height: 22px;
                line-height: 22px;
            }
        }

        .ant-select-dropdown {
            background: ${V.bordered.backgroundColor};
        }
    }
`, Gl = (e) => {
  var o = { ...e, itemRender: (a, i, l) => e.itemRender ? e.itemRender(a, i, l) : l, className: `${e.bordered ? "kit-pagination-bordered" : ""} ` + (e.className || "") };
  return t.createElement(Gi, { ...o });
}, Ti = k.div`
    margin-left: 4px;
    display: grid;
    grid-template: 'checkbox actions';

    .kit-collapse-header-extra-checkbox {
        grid-area: checkbox;
        align-self: center;
    }

    .kit-collapse-header-extra-actions {
        margin-left: 8px;
        grid-area: actions;
        display: grid;
        grid-template-columns: auto auto;

        > div:nth-child(2) {
            margin-left: 8px;
        }
    }
`, qr = ({ onSelectChange: e, actions: o }) => t.createElement(Ti, null, e !== void 0 && t.createElement("div", { className: "kit-collapse-header-extra-checkbox" }, t.createElement(ve, { onClick: (a) => a.stopPropagation(), onChange: (a) => {
  e && e(a);
} })), (() => {
  if (o !== void 0) {
    const a = o[0] || null, i = o.length <= 2 && o[1] ? o[1] : null;
    return (o == null ? void 0 : o.length) && t.createElement("div", { className: "kit-collapse-header-extra-actions" }, a && t.createElement(ne, { title: a.label }, t.createElement(te, { type: "segmented", icon: a.icon, onClick: (l) => {
      l.stopPropagation(), a.onClick && a.onClick(l);
    } })), i && t.createElement(ne, { title: i.label }, t.createElement(te, { type: "segmented", icon: i.icon, onClick: (l) => {
      l.stopPropagation(), i.onClick !== void 0 && i.onClick(l);
    } })), 2 < o.length && t.createElement("div", { onClick: (l) => {
      l.stopPropagation();
    } }, t.createElement(nr, { menu: { items: (() => {
      var l;
      if (o !== void 0 && o.length !== 0)
        return (l = [...o]).splice(0, 1), l.map((c, s) => ({ key: s, icon: c.icon, label: c.label, onClick: c.onClick }));
    })() } }, t.createElement(ne, { title: "More" }, t.createElement(te, { type: "segmented", icon: t.createElement(Fr, null) })))));
  }
})()), Xe = (qr.displayName = "KitHeaderExtra", Jr), Ri = (Xe.displayName = "KitCollapse", Xe.Header = Pr, Xe.HeaderExtra = qr, k(Cr)`
    border-radius: ${({ $rounded: e, $theme: o }) => e ? o.border.radius.rounded : o.border.radius.default}px;
    border: ${({ $bordered: e, $theme: o }) => e ? "1px solid " + o.colors.border.default : o.colors.border.none};

    + .ant-image-mask {
        border-radius: ${({ $rounded: e, $theme: o }) => e ? o.border.radius.rounded : o.border.radius.default}px;
        border: ${({ $bordered: e, $theme: o }) => e ? "1px solid " + o.colors.border.default : o.colors.border.none};
    }
`), Bi = ({ rounded: e, bordered: o, ...a }) => {
  var i = Z().theme;
  return t.createElement(Ri, { $theme: i.components.Image, $rounded: e, $bordered: o, ...a });
}, je = Bi, Ui = (je.PreviewGroup = Cr.PreviewGroup, je.displayName = "KitImage", G.div`
    display: grid;
    grid-template-columns: ${({ $gridTemplateColumns: e }) => e};
    align-items: center;
    min-height: 75px;
    padding: 0px 8px;
    background-color: ${({ $theme: e }) => e.itemList.colors.background.default};
    box-shadow: 0px 3px 8px 0px rgba(0, 0, 0, 0.1);
    border: 1px solid ${({ $theme: e }) => e.itemList.colors.border.default};
    border-radius: 7px;

    &:hover {
        border: 1px solid ${({ $theme: e }) => e.itemList.colors.border.hover};
    }

    &:focus {
        border-style: dashed;
    }

    &.kit-item-list-clickable {
        cursor: pointer;
    }

    .kit-item-list-picture-container {
        width: 50px;
        height: 50px;
        display: flex;
        align-items: center;
        justify-content: center;
        overflow: hidden;

        &:not(:first-child) {
            margin: 0px 0px 0px 10px;
        }

        &.noBorder {
            border-color: transparent;
        }

        &.kit-item-list-icon {
            .anticon {
                font-size: 2rem;
                text-align: center;
                width: 100%;
                height: 100%;
                display: inline-flex;
                align-items: center;
                justify-content: center;
            }
        }

        .kit-item-list-image-container {
            display: flex;
            align-items: center;
            justify-content: center;

            img {
                width: auto;
                max-width: 100%;
                max-height: 100%;
            }
        }
    }

    .kit-item-list-text-container {
        display: flex;
        padding: 10px 20px;
        flex-direction: column;
        align-items: flex-start;
        flex: 1 0 0;

        &.kit-item-list-text-container-with-gap {
            gap: 5px;
        }

        .kit-item-list-text {
            margin-bottom: 0px;

            &.kit-item-list-title {
                font-weight: ${({ $theme: e }) => e.title.typography.fontWeight};
                font-size: ${({ $theme: e }) => e.title.typography.fontSize}px;
                color: ${({ $theme: e }) => e.title.colors.default};
            }

            &.kit-item-list-description {
                font-weight: ${({ $theme: e }) => e.description.typography.fontWeight};
                font-size: ${({ $theme: e }) => e.description.typography.fontSize}px;
                color: ${({ $theme: e }) => e.description.colors.default};

                .ant-typography-expand {
                    visibility: hidden;
                }
            }
        }

        .kit-item-list-description-container {
            position: relative;
        }

        .kit-item-list-description-collexp {
            color: ${({ $theme: e }) => e.collexp.colors.default};

            &:hover {
                color: ${({ $theme: e }) => e.collexp.colors.hover};
            }

            &.kit-item-list-description-collapse {
                float: right;
            }

            &.kit-item-list-description-expand {
                position: absolute;
                right: 0;
                bottom: 0;
            }
        }
    }

    .kit-item-list-tag {
        &:last-child .ant-tag {
            margin-inline-end: 0px;
        }

        &:not(:last-child) .ant-tag {
            margin-inline-end: 10px;
        }
    }

    .kit-item-list-rafter {
        color: ${({ $theme: e }) => e.rafter.colors.default};
        font-size: 14px;

        &:hover {
            color: ${({ $theme: e }) => e.rafter.colors.hover};
            cursor: pointer;
        }
    }

    &.kit-item-list-disabled {
        background-color: ${({ $theme: e }) => e.itemList.colors.background.disabled};
        border: 1px solid ${({ $theme: e }) => e.itemList.colors.border.disabled};
        pointer-events: none;

        .kit-item-list-text-container {
            .kit-item-list-text {
                &.kit-item-list-title {
                    color: ${({ $theme: e }) => e.title.colors.disabled};
                }

                &.kit-item-list-description {
                    color: ${({ $theme: e }) => e.description.colors.disabled};
                }
            }

            .kit-item-list-description-collexp {
                color: ${({ $theme: e }) => e.collexp.colors.disabled};
            }
        }

        .kit-item-list-tag {
            opacity: 0.35;
        }
    }
`), Yi = ({ title: e, description: o, picture: a, onSelectChange: i, tagNumber: l, onRafterClick: c, isDisabled: s = !1, onClick: p, className: h, ...m }) => {
  var x = Z().theme;
  const [K, S] = ue(!1), [A, T] = ue(!1), C = e !== void 0, U = o !== void 0, w = a !== void 0, H = l !== void 0, ce = p !== void 0, ee = i !== void 0, O = c !== void 0;
  return t.createElement(Ui, { className: (h = h, (h += s ? " kit-item-list-disabled" : "") + (ce ? " kit-item-list-clickable" : "")), $theme: x.components.ItemList, $gridTemplateColumns: (h = "", (h += ee ? " min-content" : "") + (w ? " min-content" : "") + " minmax(0px, auto)" + (H ? " min-content" : "") + (O ? " min-content" : "")), onClick: (g) => {
    g.stopPropagation(), p && p();
  }, ...m }, ee && t.createElement("div", null, t.createElement(ve, { disabled: s, onClick: (g) => g.stopPropagation(), onChange: (g) => {
    i && i(g);
  } })), (() => {
    var g = a;
    if (!g || !g.type)
      return null;
    let _ = !1, D = {}, R = "kit-item-list-picture-container";
    switch (g.type.displayName) {
      case "KitImage":
        D = { preview: { mask: t.createElement(rr, null) }, width: "100%", height: "100%", bordered: !0, rounded: !0, rootClassName: (g.props.rootClassName || "") + " kit-item-list-image-container" }, R += " kit-item-list-image";
        break;
      case "KitIcon":
        _ = !0, D = { on: !0 }, R += " kit-item-list-icon";
    }
    var se = t.cloneElement(g, D);
    return t.createElement("div", { className: R + " " + (_ ? "noBorder" : "") }, se);
  })(), (x = "kit-item-list-text-container", x += C && U ? " kit-item-list-text-container-with-gap" : "", t.createElement("div", { className: x }, t.createElement(M.Text, { className: "kit-item-list-text kit-item-list-title", size: "large", weight: "bold", ellipsis: { rows: 1, tooltip: !0 } }, e), t.createElement("div", { className: "kit-item-list-description-container" }, t.createElement(M.Paragraph, { className: "kit-item-list-text kit-item-list-description", size: "large", weight: "regular", ellipsis: !K && { rows: 2, expandable: !0, onEllipsis: () => T(!0) } }, o, K && t.createElement(M.Link, { className: "kit-item-list-description-collexp kit-item-list-description-collapse", onClick: (g) => {
    g.stopPropagation(), S(!1);
  } }, "Less")), A && !K && t.createElement(M.Link, { className: "kit-item-list-description-collexp kit-item-list-description-expand", onClick: (g) => {
    g.stopPropagation(), S(!0);
  } }, "More")))), H && t.createElement("div", { className: "kit-item-list-tag" }, t.createElement(we, { color: "blue" }, l)), O && t.createElement("div", { className: "kit-item-list-rafter", onClick: (g) => {
    g.stopPropagation(), c && c();
  } }, t.createElement(Kr, null)));
}, zi = (Yi.displayName = "KitItemList", k(bo)`
    .ant-tree-treenode {
        padding: 0;
        font-weight: ${({ $theme: e }) => e.treenode.typography.fontWeight};

        &:hover {
            background-color: ${({ $theme: e }) => e.treenode.colors.background.hover};
            color: ${({ $theme: e }) => e.treenode.colors.typography.hover};
        }

        &.ant-tree-treenode-selected {
            background-color: ${({ $theme: e }) => e.treenode.colors.background.selected};
        }

        &.ant-tree-treenode-disabled {
            color: ${({ $theme: e }) => e.treenode.colors.typography.disabled};

            &:hover:not(.ant-tree-treenode-selected) {
                background-color: inherit;
            }

            .ant-tree-node-content-wrapper {
                color: ${({ $theme: e }) => e.treenode.colors.typography.disabled};
            }
        }

        .ant-tree-indent {
            background-color: ${({ $theme: e }) => e.treenode.colors.background.indent};
        }

        .ant-tree-switcher {
            min-height: 32px;
            line-height: 32px;
        }

        .ant-tree-checkbox {
            align-self: center;
            margin-top: 0px;
            margin-inline-end: 0px;

            .ant-tree-checkbox-inner {
                background-color: ${({ $theme: e }) => e.checkbox.colors.background.default};
                border: 1px solid ${({ $theme: e }) => e.checkbox.colors.border.default};
            }

            .ant-tree-checkbox-inner:after {
                border-color: ${({ $theme: e }) => e.checkbox.colors.icon.default};
            }

            &:hover .ant-tree-checkbox-inner {
                border: 1px solid ${({ $theme: e }) => e.checkbox.colors.border.hover};
            }

            &.ant-tree-checkbox-checked {
                .ant-tree-checkbox-inner:after {
                    border-color: ${({ $theme: e }) => e.checkbox.colors.icon.checked};
                }

                .ant-tree-checkbox-inner {
                    background-color: ${({ $theme: e }) => e.checkbox.colors.background.checked.default};
                    border: 1px solid ${({ $theme: e }) => e.checkbox.colors.border.checked.default};
                }

                &:hover .ant-tree-checkbox-inner {
                    background-color: ${({ $theme: e }) => e.checkbox.colors.background.checked.hover};
                    border-color: ${({ $theme: e }) => e.checkbox.colors.border.checked.hover};
                }
            }

            &.ant-tree-checkbox-indeterminate {
                &:not(.ant-tree-checkbox-disabled) {
                    .ant-tree-checkbox-inner {
                        background-color: ${({ $theme: e }) => e.checkbox.colors.background.checked.default};
                        border-color: ${({ $theme: e }) => e.checkbox.colors.border.checked.default};
                    }

                    &:hover .ant-tree-checkbox-inner {
                        background-color: ${({ $theme: e }) => e.checkbox.colors.background.checked.hover};
                        border-color: ${({ $theme: e }) => e.checkbox.colors.border.checked.hover};
                    }

                    .ant-tree-checkbox-inner:after {
                        width: 8px;
                        height: 0px;
                        border: 1px solid white;
                        transform: translate('-4px', '-1px');
                    }
                }
            }

            &.ant-tree-checkbox-disabled {
                color: ${({ $theme: e }) => e.checkbox.colors.icon.disabled};

                .ant-tree-checkbox-inner {
                    background-color: ${({ $theme: e }) => e.checkbox.colors.background.disabled};
                    border: 1px solid ${({ $theme: e }) => e.checkbox.colors.border.disabled};
                }

                .ant-tree-checkbox-inner:after {
                    border-color: ${({ $theme: e }) => e.checkbox.colors.icon.disabled};
                }
            }
        }

        .ant-tree-node-content-wrapper {
            transition: none;
            background-color: inherit;
            min-height: 32px;
            line-height: 32px;
            padding: 0 8px;

            .ant-tree-iconEle {
                min-height: 32px;
                line-height: 32px;
                margin-inline-end: 10px;
                font-size: 16px;
            }

            &:hover,
            &.ant-tree-node-selected {
                background-color: inherit;
                transition: none;
            }
        }

        &.ant-tree-treenode-leaf-last {
            .ant-tree-switcher-leaf-line:before {
                height: 18px !important;
            }
        }

        .ant-tree-switcher-leaf-line {
            &:after {
                height: 18px;
            }
        }
    }
`), Wi = ({ ...e }) => {
  var o = Z().theme;
  return t.createElement(zi, { $theme: o.components.Tree, ...e, blockNode: !0, draggable: !1, onDragEnd: void 0, onDragEnter: void 0, onDragLeave: void 0, onDragOver: void 0, onDragStart: void 0, onDrop: void 0 });
}, Vi = (Wi.displayName = "KitTree", k(fo)`
    .ant-tabs-extra-content {
        .ant-btn {
            border-radius: ${({ $theme: e }) => e.extraContent.border.radius}px;
            border: 1px solid ${({ $theme: e }) => e.extraContent.colors.border.default};
            padding: ${({ $theme: e }) => e.extraContent.border.radius};
            height: 34px;
            color: ${({ $theme: e }) => e.extraContent.colors.typography.default};
        }
    }

    &.ant-tabs-top,
    &.ant-tabs-bottom {
        .ant-tabs-nav .ant-tabs-extra-content:first-child {
            padding-left: 0px;
            padding-right: ${({ $theme: e }) => e.extraContent.margin.horizontal}px;
        }

        .ant-tabs-nav .ant-tabs-extra-content:last-child {
            padding-left: ${({ $theme: e }) => e.extraContent.margin.horizontal}px;
            padding-right: 0px;
        }
    }

    &.ant-tabs-left,
    &.ant-tabs-right {
        .ant-tabs-nav .ant-tabs-extra-content:first-child {
            padding-top: 0px;
            padding-bottom: ${({ $theme: e }) => e.extraContent.margin.horizontal}px;
        }

        .ant-tabs-nav .ant-tabs-extra-content:last-child {
            padding-top: ${({ $theme: e }) => e.extraContent.margin.horizontal}px;
            padding-bottom: 0px;
        }
    }

    &.ant-tabs-left .ant-tabs-nav .ant-tabs-extra-content {
        padding-right: ${({ $theme: e }) => e.extraContent.margin.vertical}px;
    }

    &.ant-tabs-right .ant-tabs-nav .ant-tabs-extra-content {
        padding-left: ${({ $theme: e }) => e.extraContent.margin.vertical}px;
    }
`), Mi = (e) => {
  var o = Z().theme;
  return t.createElement(Vi, { $theme: o.components.Tabs, ...e });
}, Qi = (Mi.displayName = "KitTabs", G(ko)`
    &.ant-alert-with-description {
        padding-inline: 0px;
        padding-block: 0px;
        padding: 8px 16px;

        .ant-alert-message {
            margin-bottom: 6px;
        }

        .anticon.ant-alert-icon {
            margin-top: 3px;
        }
    }

    &:not(.ant-alert-banner) {
        border-radius: 7px;
    }

    padding: 8px 16px;

    .ant-alert-message {
        font-size: ${r.typography.fontSize5};
    }

    .ant-alert-description {
        font-size: ${r.typography.fontSize7};
    }

    .anticon.ant-alert-icon {
        font-size: 16px;
        margin-inline-end: 8px;
    }

    .anticon.anticon-close {
        font-size: 10px;
    }

    &.ant-alert-success {
        .anticon.ant-alert-icon {
            color: ${r.color.secondary.green.green400};
        }
    }

    &.ant-alert-info {
        .anticon.ant-alert-icon {
            color: ${r.color.primary.blue400};
        }
    }

    &.ant-alert-warning {
        .anticon.ant-alert-icon {
            color: ${r.color.secondary.yellow.yellow400};
        }
    }

    &.ant-alert-error {
        .anticon.ant-alert-icon {
            color: ${r.color.secondary.red.red400};
        }
    }
`), Zi = (e) => t.createElement(Qi, { ...e }), B = (Zi.displayName = "KitAlert", { backgroundColor: r.color.neutral.typography.white, fontFamily: r.typography.fontFamily, fontSize: { title: $.fontSize4, content: r.typography.fontSize5 }, fontWeight: { title: r.typography.boldFontWeight, content: r.typography.regularFontWeight }, itemsVerticalSpacing: 24, textVerticalSpacing: 8, icon: { successColor: r.color.secondary.green.green400, successBgColor: r.color.secondary.green.green100, errorColor: r.color.secondary.red.red400, errorBgColor: r.color.secondary.red.red100, warngingColor: r.color.secondary.orange.orange500, warngingBgColor: r.color.secondary.orange.orange100 }, borderRadius: 7, imageBorderColor: r.color.neutral.typography.black60, overlayColor: "rgba(0,0,0,.45)", boxShadow: "0 6px 16px 0 rgba(0, 0, 0, 0.08), 0 3px 6px -4px rgba(0, 0, 0, 0.12), 0 9px 28px 8px rgba(0, 0, 0, 0.05)" }), Ji = k.div`
    &.kit-modal-overlay {
        background: ${B.overlayColor};
        inset: 0px;
        position: fixed;
        display: flex;
        align-items: center;
        justify-content: center;
        z-index: 1000;
    }
`, Pi = k(Zo)`
    font-family: ${B.fontFamily};

    &.kit-modal-wrapper {
        background: ${B.backgroundColor};
        border-radius: ${B.borderRadius}px;
        box-shadow: ${B.boxShadow};
        font-size: ${B.fontSize.content}px;
        font-weight: ${B.fontWeight.content};
        padding: 32px 32px 24px 32px;
        display: flex;
        position: relative;

        .kit-modal-content-wrapper {
            flex: 0 1 100%;
            position: relative;
            display: flex;
            flex-direction: column;

            > .kit-modal-title {
                flex: 0 1 auto;
            }

            .kit-modal-content {
                flex: 1 1 auto;
            }

            .kit-modal-close {
                position: absolute;
                right: 0;
                top: 0;
            }
        }
    }

    .kit-modal-title {
        font-size: ${B.fontSize.title}px;
        font-weight: ${B.fontWeight.title};
    }

    .kit-confirm-image-wrapper  {
        width: 350px;
        height: 150px;
        /* border: 1px solid ${B.imageBorderColor}; */
        border-radius: ${B.borderRadius}px;
        background-size: cover;
        background-repeat: no-repeat;
        background-position: center;
    }

    .kit-confirm-icon {
        font-size: 24px;
        display: inline-flex;
        align-items: center;
        justify-content: center;
        box-sizing: border-box;
        width: 48px;
        height: 48px;
        padding: 12px;

        &.kit-confirm-icon-success {
            color: ${B.icon.successColor};
            background: ${B.icon.successBgColor};
        }

        &.kit-confirm-icon-warning {
            color: ${B.icon.warngingColor};
            background: ${B.icon.warngingBgColor};
        }

        &.kit-confirm-icon-error {
            color: ${B.icon.errorColor};
            background: ${B.icon.errorBgColor};
        }
    }

    .kit-modal-footer {
        margin-top: ${B.itemsVerticalSpacing}px;
    }
`, Nr = ({ title: e, style: o, width: a = "520px", height: i = "initial", ...l }) => {
  o = { ...o, content: { ...o == null ? void 0 : o.content, width: a, height: i } };
  const c = () => {
    var s;
    l.showCloseIcon && ((s = l.close) == null || s.call(l, [!0]));
  };
  return a = { ...l, style: o, className: "kit-modal-wrapper " + l.className, overlayElement: (s, p) => t.createElement(Ji, { ...s, onClick: c }, p), overlayClassName: "kit-modal-overlay " + l.overlayClassName }, t.createElement(Pi, { ...a, shouldCloseOnOverlayClick: l.showCloseIcon, onRequestClose: c }, t.createElement("div", { className: "kit-modal-content-wrapper" }, l.showCloseIcon && t.createElement(or, { className: "kit-modal-close", onClick: c }), t.createElement("div", { className: "kit-modal-title" }, e), t.createElement("div", { className: "kit-modal-content" }, l.children), l.footer && t.createElement(me, { className: "kit-modal-footer", size: B.itemsVerticalSpacing, align: "end", style: { justifyContent: "end", width: "100%" } }, l.footer)));
}, qi = ({ icon: e, title: o, firstLine: a, secondLine: i, image: l, type: c, isOpen: s, width: p, portalClassName: h, okCancel: m, onCancel: x, cancelText: K, onOk: S, okText: A, close: T, ...C }) => {
  var [U] = Po("Modal");
  let w = e;
  if (e === !0 || e === void 0)
    switch (c) {
      case "info":
        w = t.createElement(P, { icon: t.createElement(Vr, null), on: !0, className: "kit-confirm-icon" });
        break;
      case "success":
        w = t.createElement(P, { icon: t.createElement(Sr, null), on: !0, className: "kit-confirm-icon kit-confirm-icon-success" });
        break;
      case "error":
        w = t.createElement(P, { icon: t.createElement(Go, null), on: !0, className: "kit-confirm-icon kit-confirm-icon-error" });
        break;
      default:
        w = t.createElement(P, { icon: t.createElement(Mr, null), on: !0, className: "kit-confirm-icon kit-confirm-icon-warning" });
    }
  return e = m ?? c === "confirm", m = (H) => () => {
    H && H(), T == null || T();
  }, c = e && t.createElement(te, { onClick: m(x) }, K || (U == null ? void 0 : U.cancelText)), x = t.createElement(te, { primaryModal: !0, onClick: m(S) }, A || (e ? U == null ? void 0 : U.okText : U == null ? void 0 : U.justOkText)), t.createElement(Nr, { isOpen: s, showCloseIcon: !1, width: p || "350px", portalClassName: h, footer: t.createElement(t.Fragment, null, c, x), ...C }, t.createElement(me, { direction: "vertical", size: B.itemsVerticalSpacing }, l && t.createElement("div", { className: "kit-confirm-image-wrapper", style: { backgroundImage: "url(" + l + ")" } }), t.createElement(me, { size: B.itemsVerticalSpacing }, w, t.createElement(me, { direction: "vertical", size: B.textVerticalSpacing }, t.createElement("div", { className: "ant-modal-title" }, o), t.createElement("div", { className: "ant-modal-body" }, a), i && t.createElement("div", { className: "ant-modal-body" }, i)))));
};
function Ee(e) {
  var o = document.createDocumentFragment();
  const a = Jo(o);
  o = "reactPortal-" + qo();
  let i = { ...e, close: s, isOpen: !0, portalClassName: o }, l;
  function c({ okText: p, cancelText: h, ...m }) {
    clearTimeout(l), l = setTimeout(() => {
      var x = m.showSecondaryCta;
      a.render(t.createElement(kr, { theme: r }, t.createElement(qi, { ...m, appElement: document.body, okCancel: x, parentSelector: () => document.body, okText: p, cancelText: h })));
    });
  }
  function s(...p) {
    p = (p = [p]).some((h) => h && h.triggerCancel), e.onCancel && p && e.onCancel(), a.unmount();
  }
  return c(i), { destroy: s, update: function(p) {
    c(i = typeof p == "function" ? p(i) : { ...i, ...p });
  } };
}
function Ni(e) {
  return { ...e, type: "warning" };
}
function Xi(e) {
  return { ...e, type: "info" };
}
function Li(e) {
  return { ...e, type: "success" };
}
function Hi(e) {
  return { ...e, type: "error" };
}
function Oi(e) {
  return { ...e, type: "confirm" };
}
const be = Nr, br = (be.displayName = "KitModal", be.info = function(e) {
  return Ee(Xi(e));
}, be.success = function(e) {
  return Ee(Li(e));
}, be.error = function(e) {
  return Ee(Hi(e));
}, be.warning = function(e) {
  return Ee(Ni(e));
}, be.confirm = function(e) {
  return Ee(Oi(e));
}, { circleFontSize: $.fontSize2, circleFontWeight: r.typography.boldFontWeight }), Di = k(xo)`
    &.ant-progress-circle .ant-progress-text {
        font-size: ${br.circleFontSize}px;
        font-weight: ${br.circleFontWeight};
    }
`, ji = (e) => t.createElement(Di, { ...e }), $i = (ji.displayName = "KitProgress", 4e3), _i = ({ message: e, closable: o, ctaText: a, ctaOnClick: i, duration: l = $i }) => (Me(), wr(t.createElement(rl, { message: e, ctaText: a, ctaOnClick: i, closable: o }), { duration: l })), Me = () => {
  wr.remove();
}, el = G.div`
    display: grid;
    grid-template-columns: auto ${(e) => e.$showCtaContainer ? "min-content" : ""} ${(e) => e.$showCloseContainer ? "12px" : ""};
    column-gap: 8px;
    align-items: center;
    min-height: 24px;
    padding: 8px 16px;
    background-color: ${r.color.neutral.gray.gray700};
    border: 1px solid ${r.color.neutral.gray.gray700};
    border-radius: 7px;
    box-shadow: 0px 3px 14px 0px rgba(0, 0, 0, 0.3);
    font-family: ${r.typography.fontFamily};

    .kit-snackbar-grid-one {
        min-width: 150px;
        grid-column: 1;
    }

    .kit-snackbar-grid-two {
        grid-column: 2;
    }

    .kit-snackbar-grid-three {
        grid-column: ${(e) => e.$showCtaContainer ? "3" : "2"};
    }

    .kit-snackbar-grid {
        .kit-snackbar-title {
            color: ${r.color.neutral.gray.gray400};
            font-size: ${r.typography.fontSize5};
            font-weight: ${r.typography.regularFontWeight};
        }

        .kit-snackbar-cta-container {
            height: 23px;
            line-height: 23px;
            width: fit-content;
            padding: 0px 12px;
            border: 0.499px solid ${r.color.neutral.gray.gray200};
            border-radius: 14px;
            cursor: pointer;

            .kit-snackbar-cta {
                color: ${r.color.neutral.gray.gray200};
                font-size: 10px;
                font-weight: ${r.typography.mediumfontWeight};
                text-transform: uppercase;
            }

            &:hover {
                border-color: ${r.color.neutral.gray.gray400};

                .kit-snackbar-cta {
                    color: ${r.color.neutral.gray.gray400};
                }
            }
        }

        .kit-snackbar-close-container {
            color: ${r.color.neutral.gray.gray400};
            font-size: 12px;
            cursor: pointer;

            &:hover {
                color: ${r.color.neutral.gray.gray500};
            }
        }
    }
`, rl = ({ message: e, ctaText: o, ctaOnClick: a, closable: c }) => {
  var l = o !== void 0 && a !== void 0, c = c !== void 0;
  return t.createElement(el, { $showCtaContainer: l, $showCloseContainer: c }, t.createElement("div", { className: "kit-snackbar-grid kit-snackbar-grid-one" }, t.createElement("div", { className: "kit-snackbar-title" }, e)), l && t.createElement("div", { className: "kit-snackbar-grid kit-snackbar-grid-two" }, t.createElement("div", { className: "kit-snackbar-cta-container", onClick: () => {
    a(), Me();
  } }, t.createElement("div", { className: "kit-snackbar-cta" }, o))), c && t.createElement("div", { className: "kit-snackbar-grid kit-snackbar-grid-three" }, t.createElement("div", { className: "kit-snackbar-close-container", onClick: () => Me() }, t.createElement(or, null))));
}, Tl = _i, Rl = Me, ol = Gr;
ol.displayName = "KitSnackBarProvider";
export {
  Zi as KitAlert,
  Fl as KitApp,
  jn as KitAutoComplete,
  sr as KitAvatar,
  Aa as KitBadge,
  ui as KitBreadcrumb,
  te as KitButton,
  ve as KitCheckbox,
  Xe as KitCollapse,
  ur as KitDatePicker,
  yi as KitDivider,
  nr as KitDropDown,
  Il as KitGrid,
  Ci as KitHeader,
  P as KitIcon,
  je as KitImage,
  Ke as KitInput,
  _a as KitInputNumber,
  ci as KitItemCard,
  Yi as KitItemList,
  yr as KitMenu,
  be as KitModal,
  Gl as KitPagination,
  ji as KitProgress,
  mr as KitRadio,
  an as KitRate,
  Jn as KitSelect,
  Kl as KitSlider,
  ol as KitSnackBarProvider,
  me as KitSpace,
  Ii as KitSteps,
  Xn as KitSwitch,
  Mi as KitTabs,
  we as KitTag,
  ne as KitTooltip,
  Wi as KitTree,
  M as KitTypography,
  Pe as KitUpload,
  Rl as closeKitSnackBar,
  d as colorsPalette,
  Tl as openKitSnackBar,
  r as theme,
  $ as typographyHeaders,
  El as useKitNotification
};
