...
export const Template = ({label, tagInput, ...args}) => {
    const {options, value, placeholder, tokenSeparators, notFoundContent, placement, onSearch, ...tagArgs} = args;
    const [opts, setOpts] = useState<string[]>([]);

    const onInternalSearch = value => {
        const optionsArray = (options || argTypes.options.control.value).split(',').filter(v => v !== value);
    ...
    return <KitTag {...tagArgs}>{label || argTypes.label.control.value}</KitTag>;
};
